...
int main(int argc, char *argv[])
{
	...
	*/
	...
	/* ------------------------------------------------------------- */
	YZ_max_text = MY_MAX; Yz_max_graf = YZ_max_text; l_poz_menu = Liczba_opcji;
	MYR_MAX = MY_MAX - 1;
	Menu[L_SYS + 1] = graf_text[0];
	l_poz_menu = Liczba_opcji + 1; Yz_max_graf = Y_MAX_graf;
	...
	do
	{
		...
		{
			float xr;
			static float suma = 0; static int liczba = 0;
			// xr=RNDM(); 
			// suma+=xr; 
			// liczba++; 
...
#include "blank/hasla.c"
	...
	for(i=0; i<liczba_opcji;i++)
	 {int kod_uslugi, rek0, rekf, kolor=0, ramka=1, max_l_data_blank=100;
		kod_uslugi=Serv[i]->kod_uslugi; rek0=0; rekf=0;
...
void naglowek(unsigned int attr_logo)
 {int i, len;
  ...
  if(podsNameLen>24) {i=podsNameLen-24; X_tyt-=i;}
...
int czas_zegarowy(int *godz, int *min, int *sek)
 {time_t sek1970;
	...
	*godz=Time->tm_hour; *sek=Time->tm_sec;  *min=Time->tm_min;
...
int data_systemowa(int *dzien, int *miesiac, int *rok)
{
	...
	sek1970=time(NULL);
	Time=localtime(&sek1970);
	*dzien=Time->tm_mday; *miesiac=Time->tm_mon; *rok=Time->tm_year+1900;
	return 0;
}
...
void chk_time(void)
 {time_t sek1970, dt;
	...
	if(sek_akt!=sek1970) /* Zmiana sekundy */
	 {dt=sek1970-sek_akt; sek_akt=sek1970;
		check_agenda(dt);
	 }
 }
...
void check_services(void)
 {struct agenda *A;
	struct Service *S;
	 ...
	 A=check_queue(); if(A==NULL) return;
...
int insert_to_agenda(struct agenda *A)
 {int i, poz=-1, x, y;
	...
	{struct agenda *An;
	 ...
	 ag_size=sizeof(struct agenda); lwmall++;
...
struct agenda *service_default(int (*decyzje)(int decyzja, int kod_decyzji, int nr_dec,
 ...
 {struct agenda *A;
	...
	int size, ag_size, i, x, y, str_size;
	 ...
	 A=(struct agenda *)Malloc(size); lwmall++;
	 if(A==NULL)
		{x=m_wherex(); y=m_wherey(); setcursor(nocursor);
	 ...
	 A->S=Serv[service_no];
	 A->data=NULL;
	 ag_size=sizeof(struct agenda); str_size=(A->S)->str_size;
	 if(str_size>ag_size)
		{A->data=(A+1);
		 for(i=0;i<str_size-ag_size;i++) {*((char *)A->data+i)=0;}
		}
	 A->name=(A->S)->name; A->state=0;
	 A->mode='n'; A->number_of_calls=1;
	 A->prior=1; A->prior_plus=0;
	 A->Interval=60; A->cur_prior=A->prior;
	 A->delay=160;
	 A->status=0;
	 return A;
 }
...
void cancel_serv(struct agenda *A, int i)
 {/* struct queue *Q; */
	if(i>=0)
	 {A=SysQ[i]; if(A->mode=='t') free_service(A);
		SysQ[i]=NULL;
		return;
	 }
	if(A->mode=='t')
	 {Free(A); lwmall--;/* usuwamy servis */}
	return;
 }
	...
	for(Nrq=0,nq=-1;Nrq<=NoServisMax; Nrq++)
	 {if(SysQ[Nrq]==NULL) continue;
		...
		if(select_mode==MAX_PRIOR && A->cur_prior>max_prior)
		 {sel=Nrq; max_prior=A->cur_prior;}
		else        /* to jest gdy RAND_PRIOR */
		 {nq++; lim+=A->cur_prior; prior_lim[nq]=lim; serv_ready[nq]=Nrq;}
		break;
	 }
	...
	if(sel<0)   /* to jest gdy RAND_PRIOR */
	 {lim=lim*RND01();
		for(Nrq=0;Nrq<=nq;Nrq++)
		 {if(lim<prior_lim[Nrq]) {sel=Nrq; break;}}
	 }
	A=SysQ[sel];
...
int insert_to_queue(struct agenda *A)
 {int i, poz=-1, status;
	...
	if(poz<0)   /* szukamy wolnego miejsca */
	 {for(i=0;i<=NoServisMax; i++)
		 {if(SysQ[i]==NULL) {poz=i; break;}
			if(SysQ[i]->state==0) {cancel_serv(SysQ[i],i); poz=i; break;}
		 }
		if(poz<0)
		 {poz=i;
			if(poz>=Q_SIZE)
	...
	A->state=1;
	SysQ[poz]=A; (SysQ[poz])->last_time=sek_akt;
	SysQ[poz]->cur_prior=A->prior;
	return poz;
 }
/* ==================================================================== */
struct agenda **getAgendaPtr(int *ag_size)
 {*ag_size=AG_SIZE; report_act[ag_rep]=1; return SysA;}

struct agenda **getQueuePtr(int *q_size)
 {*q_size=Q_SIZE; report_act[q_rep]=1; return SysQ;}

void **getRepDataPtr(int *d_size)
 {int i;
	for(i=0;i<L_REP_MAX;i++) {if(report_act[i]==0) break;}
	if(i==L_REP_MAX) {*d_size=NULL; return NULL;}
	*d_size=AG_SIZE; report_act[i]=1;
	return RepDataPtr[i];
 }

void freeAgendaPtr(void) 
{
	report_act[ag_rep]=0; return;
}

void freeQueuePtr(void) {report_act[q_rep]=0; return;}
...
/*	if(A->data!=NULL) Farfree(A->data);  */
	Free(A); SysA[nr_ag]=NULL; lwmall--; /* usuwamy servis */
	...
	for(na=1, rap_no=-1;na<lzl; na++)
	 {if(SA[na]==NULL) continue;
		if((SA[na]->S)->kod_uslugi==kod_uslugi)
		 {rap_no++;
			if(nr_w_agendzie<0)
			 {if(rap_no==(*nr_rekordu)) {A=SA[na]; break;}}
			else
			 {if(na==nr_w_agendzie) {A=SA[na]; *nr_rekordu=rap_no; break;}}
		 }
	 }
	if(rap_no<0) {(*nr_rekordu)=0; return NULL;}
	if(rap_no<(*nr_rekordu)) (*nr_rekordu)=rap_no;
	return A;
 }
/* ---------- Funkcje uslugowe dla wszystkich zlecen ----------------- */

int pobierz_rekord_uslugi(int *nr_rekordu, int kod_uslugi, int ob_konc, struct agenda **As, struct agenda *Anew, struct agenda ***SA, char *adres_rek0_uslugi, char *nazwa)
 {struct agenda *A=NULL, *A0=NULL, **Ab=NULL;
	...
	*As=NULL;
	...
	for(i=0,nr=0;i<ob_konc;i++)
	 {Ab[i]=NULL;
		...
		if(A==NULL) continue;
		if((A->S)->kod_uslugi==kod_uslugi) {Ab[nr]=A; nr++;}
	 }
	if(nr==0 && Anew==NULL)
  ...
  if(Anew!=NULL)  /* zadeklarow. w sys_dekl.h */
	 {nr++; nr_rek=nr; Ab[nr_rek]=Anew;}
	else {nr_rek=((*nr_rekordu)%nr);}
	Ab[0]=Ab[nr_rek]; for(i=1;i<nrR;i++) Ab[i]=NULL;
  ...
  *nr_rekordu=nr_rek;
  return ret;
	 ...
	 {static char *status[] = { " nie wchodzi gdy poprzednie jest jeszcze w kolejce",
		 ...
		 else
		 {
			 if (Anew != NULL)
			 {
				 ...
				 size = 5, ochr = -1, raport = -1);
					  ...
					  ret = dana_koment(-1, -1, "+ ");
					  {static char *uruch[] = { "d dane", "n nastepny", "l lista", "m menu glowne", "u USUNAC", "w WPIS do agendy" }, dec = 'd';
					  dec = 'd';
...
int pokaz_listy_zlecen(struct agenda *Anew, int kod_uslugi, char *tytul)
{
	...
	struct agenda **Ab, *A;
	ret = 0;
	if (Anew != NULL) { Free(Anew); lwmall--; Anew = NULL; }
...
int decyzje_run(char decyzja, struct agenda **Aserv, struct agenda **Anew, int *nr_rekordu, int kod_uslugi, char *tytul)
{
	...
	case 'n': (*nr_rekordu)++; break;
	case 'u': if ((*Anew) != NULL) { Free(*Anew); lwmall--; *Anew = NULL; }
...
int SprawdzLog(char *uzytkownik, char *haslo)
{
	...
	if (fp == NULL) { okno_menu("Blad otwarcia pliku", 1, 0, attr, at_wpis, 10, 25, -1, " BLAD", 1); return -1; }
	else
	{
		do{
