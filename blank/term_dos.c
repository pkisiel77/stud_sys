...
/*0.0.3*/
shint x_cur, y_cur;
Ushint char_width, char_height, odstep;
shint x0_cur=0, y0_cur=0;
shint xmax_cur=5000, ymax_cur=5000;
void chk_time(void);
void check_services(void);
int cur_bgkcol, cur_color, Cursor_color, is_cursor;
unsigned int  textkol, bgkol;
shint Xl=5000, dc=1, fsize=5;
unsigned int  textkol, bgkol;
shint xg_min=0, yg_min=0, xd_max, yd_max;
...
/*1*/
shint Endgraph(void)/* ZAMKNIECIE grafiki */
{
 return 0;
}
/*2*/
void setcursor(unsigned int shape)
{ if(shape==nocursor) is_cursor=0; else is_cursor=1;
#ifdef GRAF
  if(igraf!=0) return;
#endif
}
...
{	
	HDC hdc;
	shint cc, blink;
  	textkol=attr&0x00ff; blink=attr&TERM_BLINK;
...
shint term_type(shint y, shint x, char *text, shint lenth, unsigned int attr)
{
	...
	m_gotoxy(x,y);	
	if(lenth==0) {lenth=strlen(text);xg_akt+=lenth;}
	else
	{if(lenth==1) {x_cur+=1;xg_akt++;}
	 else
	 {l=strlen(text);
	  lzn=strlen(text);
	  xg_akt+=lzn;
	  if(l<lenth)
	  {lzn=(lenth-l);xg_akt+=lzn; lzn*=1;
	   x_cur+=lzn;
	  }
	 }
	}
...
int term_printf(int y, int x, unsigned int attr, char *format,...)
{
    ...
    ReleaseDC(handle_window_device,hdc);
		...
		x_cur=xg_akt; y_cur=yg_akt;
		do
		 {if(cd!=NULL) {format=cd; format[0]='\n'; cd=NULL;}
			va_start(arg,format);
			if(format[0]=='\n')
			 {y_cur+=1;
				if(y_cur>yd_max) y_cur-=1; else  yg_akt++;
				x_cur=x0_cur; xg_akt=x_min; format++;
			 }
			if(format[0]=='\r') {x_cur=x0_cur; xg_akt=x_min; format++;}
			l=strlen(format);
			for(i=0;i<l;i++)
			 {if(format[i]=='\n') {cd=&format[i]; format[i]=EOS; break;}}
			...
			do
			 {bf=NULL;
				do
				 {l=cx_char;
					i=x_cur+l; if(i<xd_max) break;
					l=strlen(bufor);
					if(bf!=NULL) bufor[l]=c;
					l=l-1; bf=&bufor[l]; c=bufor[l]; bufor[l]=EOS;
					i=x_cur;
				 }while(l>0);
			    lzn=strlen(bufor);
				...
				if(bf!=NULL)
				 {y_cur+=1; yg_akt++;
					if(y_cur>yd_max) {y_cur-=1; yg_akt--;}
					Xl=x_cur; x_cur=x0_cur; bufor=bf; bufor[0]=c;
				 }
			 }while(bf!=NULL);
			va_end(arg);
		 }while(cd!=NULL);
...
/*9*/
shint m_wherex(void)
{return xg_akt;} // !!! Uwaga powinno byæ x_akt le nie dzia³a wiêc xg_akt;
/*10*/
shint m_wherey(void)
{return yg_akt;}
/*11*/
	...
	if(igraf!=0)
	 {
		...
		setcolor(cur_color);
		x_cur=x_akt*char_width; y_cur=y_akt*odstep;
		...
		if(alarmclock>0)
		 {ticks=clock();  /* czas w taktach zegara CLK_TCK l.taktow na sek*/
			...
			if(dtck>=lticks_sound)
			 {if(flaga==0) {flaga=1; sound(500);}
				else {flaga=0; nosound();}
				ticka=ticks;
			 }
		 }
        ...
        DispatchMessage (&term_msg) ;
		 if(term_msg.message==WM_KEYDOWN)  
			 { znak=term_msg.wParam;
			       // znaki steruj¹ce <-> 
	               if(znak==38) {znak=0;n_znak=UP;}
				   if(znak==40) {znak=0;n_znak=DOWN;}
				   if(znak==39) {znak=0;n_znak==FFR;}
                   if(znak==37) {znak=0;n_znak==BBS;}
				   // F1...F12
				   if(znak==112) {znak=0;n_znak=FONT1;}
                   if(znak==113) {znak=0;n_znak=FONT2;}
                   if(znak==114) {znak=0;n_znak=FONT3;}
				   if(znak==115) {znak=0;n_znak=FONT4;}
                   if(znak==116) {znak=0;n_znak=FONT5;}
                   if(znak==117) {znak=0;n_znak=FONT6;}
				   if(znak==118) {znak=0;n_znak=FONT7;}
                   if(znak==119) {znak=0;n_znak=FONT8;}
                   if(znak==120) {znak=0;n_znak=FONT9;}
				   if(znak==121) {znak=0;n_znak=FONT10;}
                   if(znak==122) {znak=0;n_znak=FONT11;}
                   if(znak==123) {znak=0;n_znak=FONT12;}
			 }
            // 2. MyszWM_RBUTTONDOWN
			if(term_msg.message==WM_LBUTTONDOWN)	{znak=0;n_znak=UP;}
			if(term_msg.message==WM_RBUTTONDOWN)	{znak=0;n_znak=DOWN;}
			// Enter i Esc ???
		if(znak!=1)
		{ticks=clock();  // czas w taktach zegara CLK_TCK l.taktow na sek
		 dtck=ticks-tickp;
