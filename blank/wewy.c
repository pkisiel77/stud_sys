...
void open_konsole_mouse(shint fd_in);
shint BUF_com_size=0, nr_komunik=-1;
void *BUF_com=NULL;
...
shint (*odbior_komunik)(void *b, pid_t a)=NULL;
#undef buttons
...
struct baza_grafik
 {shint Lgrafs;
	...
	void *(*akcja_grafik)(shint typ_kom, shint kto, char *pdane, char *Text,
						void *(*Dane_Wykr)(void *Rd), void *R);
	shint report[MAX_GRAFS];
 }BGR;
...
{char cox;
 ...
 if(co < 3)
	{term_printf(y_akt,x_akt,attryb,"Brak wzorca funkcji mgetch() %s ??. Nacisnij dowolny klawisz",monit);
	 Get_Char(); /* dev_mode(0,opcje,_DEV_MODES);*/ return 0;
	}
 cox=m_wherex();
 do{x_akt=cox;
		term_printf(y_akt,x_akt,attryb," <%s> ?? ",monit); x_akt+=strlen(monit);
		cco=Get_Char(); co=cco&0x00FF;
    if(co==Esc) {if(ABORT==1) mAbort(); /* dev_mode(0,opcje,_DEV_MODES);*/ return co;}
		else
		 {if(co==EOS) {Get_Char(); continue;}
      if(co==CR) continue;
...
*/
 {shint ls;
  char f=0,male=0;
  if((opcje&1)!=0) f=1;
  if((opcje&2)!=0) male=1; text[0]=EOS;
...
*/
 {shint l_danych, z0, x_pocz, dlm;
  ...
  if(forms==NULL)
   {yplus(); x_akt=x_akt0;
    term_printf(y_akt,x_akt,attryb,"mscanf(%s)"); yplus();
...
/*  larg=nargs()-1; */
	m0=*monit; *forms=EOS; dlm=strlen(monit);
	x_pocz=x_akt;
P:text[0]=EOS;
  x_akt=x_pocz; term_cur(y_akt, x_akt);
  goto D;
B:x_akt=x_pocz+dlm; term_cur(y_akt, x_akt);
D:z0=strlen(text);
	adres_danej=&stos;
  adres_danej++;
  form=forms;
	if(*monit=='\n') {yplus(); x_akt=x_akt0; monit++;}
  if(*monit=='\r') {x_akt=x_akt0; monit++;}
  m0=*monit;
	if(czytaj_we(monit,text,z0,x_maxOkna-x_akt0-strlen(monit))<-1)
	 {text[0]=EOS; *forms='%'; *monit=m0;
    return -1;
   }
  (*forms)='%'; *monit=m0;
	ksl=text;
  if(*ksl==EOS)
   {if(nl==1 && *monit!='\n') {yplus(); x_akt=x_akt0;}
		if(def==1)  {*forms='%'; *monit=m0;
                 return 0;
                }
    else {*forms=EOS; goto P;}
	 }
  do
	 {form++;
    f=*form;
...
*/
    f=*form;
			if(f=='s') {ff=-1; if(ksl==text) sl=ksl; else sl=szukaj_slowa(ksl,ff);}
		  else {ff=2; sl=szukaj_slowa(ksl,ff);}
    if(ff>=0) ksl=szukaj_konca(sl,ff,0);
		ff=1;
    switch(f)
    {case 's': przepisz(*adres_danej,sl, sizeof(sl)); break;
     case 'd':
      if(znaki_liczby(sl,0)==1) *((shint*)(*adres_danej))=atoi(sl);
      else {if(BLAD==1) {yplus(); term_printf(y_akt,x_akt0,attryb,
                                     "Bledna dana (%s) typu %c",sl,f);
                         yplus(); x_akt=x_akt0;
                        }
						else {bip(1); monit[0]=EOS; goto B;}
            *forms=EOS;
						goto P;
           }
			break;
     case 'e':
      if(znaki_liczby(sl,2)==1)
           {a=atof(sl); *((double*)(*adres_danej))=a;}
      else {if(BLAD==1)
						 {yplus(); komunikat(MY_MAX,X_L0,attryb,"Bledna dana (%s) typu %c",sl,f);}
						else {bip(1); monit[0]=EOS; goto B;}
            *forms=EOS;
            goto P;
					 }
     ...
     case 'f':
      ...
      else {if(BLAD==1) {yplus(); term_printf(y_akt,x_akt0,attryb,"Bledna dana (%s) typu %c",sl,f);
                         yplus(); x_akt=x_akt0;
                        }
						else {bip(1); monit[0]=EOS; goto B;}
            *forms=EOS;
     ...
     default:
			...
			cc=Get_Char(); f=cc&0x00FF;
      if(f=='q') {return(0);}
      else {yplus(); x_akt=x_akt0;};
    }
	 adres_danej++;  l_danych++;
   form=szukaj(form,'%');
...
/* f - format konwersji slowa:
       ...
       l  - tylko liczby  (slowo[]= adres testow stanowiacych kolejne liczby
                                    NULL gdy tekst nie jest liczba
*/
 {shint l_danych, z0, x_pocz, dlm, ret;
  ...
  double a;
  l_danych=0;
  z0=strlen(text); if(z0>MXR_MAX) text[MXR_MAX]=EOS;
	monit=stos;
  if(*monit=='*') monit++;
  if(*monit=='+') {monit++; nl=1;}
	if(*monit=='#')
...
/*  larg=nargs()-1; */
	m0=*monit; *forms=EOS; dlm=strlen(monit);
  x_pocz=x_akt;
P:x_akt=x_pocz; term_cur(y_akt, x_akt);
  ifmon=1;
	goto D;
B:x_akt=x_pocz+dlm; term_cur(y_akt, x_akt);
  ifmon=0;
D:z0=strlen(text);
  ...
  form=forms;
	if(ifmon==1)
   {if(*monit=='\n') {yplus(); x_akt=x_akt0; monit++; dlm--;}
		if(*monit=='\r') {x_akt=x_akt0; monit++; dlm--;}
...
*/
    ...
    ff=1;
    switch(f)
		 {case 's': strcpy(*adres_danej,sl); break;
			case 'c': *adres_danej[0]=sl[0];   break;
			case 'd':
			 if(znaki_liczby(sl,0)==1) *((shint*)(*adres_danej))=atoi(sl);
			 else {if(BLAD==1) {yplus(); komunikat(MY_MAX,X_L0,attryb,
																		 "Bledna dana (%s) typu %c",sl,f);
												 yplus(); x_akt=x_akt0;
												}
						else {bip(1); monit[0]=EOS; goto B;}
...
char *szukaj(char *text,char znak)
 {while((*text)!=EOS){if((*text)!=znak) text++; else return text;};
	return NULL;
 }   /*...end szukaj().....*/
/* ----------------------------------------------------------------------
shint szukaj_znak(char *text,char znak)
 {shint i=0;
...
shint czytaj_we(char *monit, char *text, shint lz0, shint liczba_znakow)
 {shint lz,nEOS=0,lzp=0,x, x_pocz, start=0, ccx, i;
	shint y_cursor, x_cursor,  x_cur0, x_curM, y_cur0, y_curM;
	unsigned int c,cc, attr_old;
	lticks_min=kbtickm;
	if(x_maxOkna-x_akt+1<liczba_znakow) liczba_znakow=x_maxOkna-x_akt+1;
	nEOS=strlen(text);
	if(nEOS>=liczba_znakow) {text[liczba_znakow]=EOS; nEOS=liczba_znakow;}
	y_cursor=y_akt;
	if(monit!=NULL)
	...
	else {x_akt=x_kursor; lz0=x_akt-x_pocz; start=1;}
	x_kursor=-1;
	if(x_akt>x_maxOkna) x_akt=x_maxOkna;
	if(lz0>=liczba_znakow) lz0=liczba_znakow-1;
	if(HELP==0) /* && EDIT==1) */
	 {ccx=strlen(text)-1;
		if(ccx>0) {while(ccx>0 &&(text[lz0-ccx]==' ')){ccx--;} x_akt-=ccx; lz0-=ccx;}
		if(lz0<0) lz0=0;
   }
/*  else {if(HELP!=0) x_akt+=lz0;} */
	cc=0; lz=lz0; lzp=0; term_cur(y_cursor,x_akt);
	do
		{term_flush();
		 x_cursor=m_wherex(); y_cursor=m_wherey();
		 x_cur0=x_akt0; x_curM=x_maxOkna;  y_cur0=y_akt0; y_curM=y_maxOkna;
		 attr_old=attryb;
/* -------------------- tu moze wejsc RTkernel ------------------------- */
		 cc=GET_char(); c=cc&0x00FF;
/* --------------------------------------------------------------------- */
		 x_akt0=x_cur0; x_maxOkna=x_curM;  y_akt0=y_cur0; y_maxOkna=y_curM;
		 attryb=attr_old;
...
/*     if(c==NLCR) c=CR/ *=GET_char()*/;
		 if(c==CR)
			{clear_cur(text[lz]);
			 if(start==0) {lticks_min=Ltickm; return -1;}
			 cc=c; continue;
			}
		 if(c==BS)
			{if(lz==0)
				{if(nEOS==1) {nEOS=0; goto DEL;}
				 else continue;
				}
			 lzp=lz;
		 ...
		 if(c==Esc)
			{if(ABORT==1) {mAbort(); break;}
			 else {if(EDIT==0) {text[0]=Esc; text[1]=EOS;}
						 lticks_min=Ltickm; return -2;
						}
			}
		 ...
		 if(c==EOS || c==SPEC)
			{c=Get_char(); /*c=cc&0x00FF*/;
			 ...
			 if(c==FFR) {if(lz>=nEOS) continue;
									 if(x_akt>=x_maxOkna) continue;
									 clear_cur(text[lz]);
									 lz++; if(lz==liczba_znakow) lz--;
									 xplus(); term_cur(y_akt,x_akt); /* m_gotoxy(x_akt,y_cursor); */
									 continue;
									}
			 if(c==Home)
				{
			 ...
			 if(c==End)
				{/*fprintf(mystderr,"lz=%d nEOS=%d, dltx=%d\n",lz,nEOS,strlen(text));*/
				 clear_cur(text[lz]);
				 lzp=lz; x_akt=x_pocz+lz;
				 for(lz=lzp;lz<nEOS;lz++)
					{term_type(y_akt,x_akt,&text[lz],1,attryb); xplus();}
				 lz=nEOS;
			 ...
			 if(c==UP)
				{if(EDIT==1)
					{if(y_cursor==y_min) continue;
					 y_akt--; x_kursor=m_wherex(); lticks_min=Ltickm; return nEOS;
					}
				 cc=text[lz];
				 if(cc<'0' || cc>'9') continue;
				 text[lz]++; if(text[lz]>'9') text[lz]='0';
				 goto INC;
				}
			 if(c==DOWN)
				{if(EDIT==1)
				 ...
				 cc=text[lz];
				 if(cc<'0' || cc>'9') continue;
				 text[lz]--; if(text[lz]<'0') text[lz]='9';
INC:  	 term_type(y_akt,x_akt,&text[lz],1,attryb);
				 term_cur(y_akt,x_akt);
				 start=1;
				 if(HELP==0) continue;
				 HELP=2; x_kursor=m_wherex(); lticks_min=Ltickm; return nEOS;
				}
			 if(c==Del)
				{if(lz>=nEOS) continue;
DEL:     lzp=lz; x=x_akt; Clr_eol(); start=1;
				 if(nEOS>0)
...
*/
		 ...
		 if(lz>=liczba_znakow) {lz=liczba_znakow-1; continue;}
		 c=cc;
		 if(isprint(cc))
			{if(INSERT<0)
				{lzp=lz;
				 if(nEOS>=liczba_znakow)
					{nEOS=liczba_znakow;
					 if(text[0]!=' '&&text[nEOS]!=' ') {bip(0); continue;}
					 m_gotoxy(x_pocz,y_cursor);
/*					 term_cur(y_akt, x_akt); */
					 for(lz=0;lz<lzp;lz++)
						{text[lz]=text[lz+1];
						 term_type(y_cursor,x_akt,&text[lz],1,attryb); xplus();
						}
					}
				 else
					{lzp=lz;
					 for(lz=nEOS;lz>lzp;lz--) text[lz]=text[lz-1];
					 nEOS++; text[nEOS]=EOS; x=x_akt+1;
					 for(lz=lzp+1;lz<nEOS;lz++,x++) term_type(y_cursor,x,&text[lz],1,attryb);
					}
				 lz=lzp;
				}
			 text[lz]=c; /* term_cur(y_cursor,x_akt); */
			 term_type(y_cursor,x_akt,(char *)&cc,1,attryb); xplus();
			 term_cur(y_cursor,x_akt);
			 lz++; if(lz==liczba_znakow) {nEOS=lz; text[nEOS]=EOS; lz--;}
			 start=1;
/*       fprintf(mystderr," lz=%d nEOS=%d c=%c tx[lz]=%c ",lz,nEOS,c,text[lz]);*/
			 if(nEOS<lz) {nEOS=lz; text[nEOS]=EOS;}
			}
		} while (cc!=CR);
	text[nEOS]=EOS; lticks_min=Ltickm;
	return nEOS;
 }  /*....end czytaj_we()....*/

/*------------------------------------------------------------------------*/
shint czytaj_has(char *text, shint lz0, shint liczba_znakow)
 {shint lz,nEOS=0,lzp=0,x, x_pocz, start=0, i;
	shint y_cursor, x_cursor,  x_cur0, x_curM, y_cur0, y_curM;
	unsigned int c,cc, attr_old;
	lticks_min=kbtickm;
	if(x_maxOkna-x_akt+1<liczba_znakow) liczba_znakow=x_maxOkna-x_akt+1;
...
/*  term_type(y_cursor,m_wherex(),text,0,attryb); / * ###### */
	...
	if(x_akt>x_maxOkna) x_akt=x_maxOkna;
	if(lz0>=liczba_znakow) lz0=liczba_znakow-1;
	lz=lz0; lzp=0; term_cur(y_akt,x_akt);
	do
		{term_flush();
		 x_cursor=x_akt; y_cursor=m_wherey();
		 x_cur0=x_akt0; x_curM=x_maxOkna;  y_cur0=y_akt0; y_curM=y_maxOkna;
		 attr_old=attryb;
		 cc=Get_Char(); c=cc&0x00FF;
		 x_akt0=x_cur0; x_maxOkna=x_curM;  y_akt0=y_cur0; y_maxOkna=y_curM;
...
/*     if(c==NLCR) c=CR/ *=GET_char()*/;
       ...
       if(nEOS<liczba_znakow)
		 ...
		 if(c==Esc)
			{if(ABORT==1) {mAbort(); break;}
			 else {text[0]=Esc; text[1]=EOS; lticks_min=Ltickm;  return -2;}
			}
		 if(c==EOS || c==SPEC)
			{c=Get_char(); /*c=cc&0x00FF*/;
			 if(c==BBS || c==LEFT)
...
#ifdef GRAF
				 if(igraf!=0) backspace(&text[lz]);
				 else {m_gotoxy(x_pocz+lz,y_akt);  term_cur(y_akt,x_akt);}
#else
			 ...
			 if(c==FFR) {if(lz>=nEOS) continue;
									 if(x_akt>=x_maxOkna) continue;
                   lz++; if(lz==liczba_znakow) lz--;
			 xplus(); m_gotoxy(x_akt,y_akt); term_cur(y_akt,x_akt);
									 continue;
                  }
			 if(c==Home)
        {x_akt=x_pocz;
...
#endif
				 lz=0;
			 m_gotoxy(x_akt,y_akt); term_cur(y_akt,x_akt);
         continue;
        }
       if(c==End)
				{/*fprintf(mystderr,"lz=%d nEOS=%d, dltx=%d\n",lz,nEOS,strlen(text));*/
         lzp=lz; x_akt=x_pocz+lz;
				 for(lz=lzp;lz<nEOS;lz++)
          {term_type(y_akt,x_akt,"*",1,attryb); xplus();}
         lz=nEOS;
         if(lz==liczba_znakow) lz--;
         term_cur(y_akt,x_akt);
				 continue;
       ...
       if(c==DOWN)
        {cc=text[lz];
         if(cc<'0' || cc>'9') continue;
         text[lz]--; if(text[lz]<'0') text[lz]='9';
INC:     term_type(y_akt,x_akt,"*",1,attryb);
...
DEL:     lzp=lz; x=x_akt; Clr_eol(); start=1;
       ...
       if(c==Ins) {INSERT=-INSERT; continue;}
       continue;
      }
     if(lz>=liczba_znakow) {lz=liczba_znakow-1; continue;}
     c=cc;
     if(isprint(cc))
      {if(INSERT<0)
				{lzp=lz;
				 if(nEOS>=liczba_znakow)
					{nEOS=liczba_znakow;
					 if(text[0]!=' '&&text[0]!=EOS) {bip(0); continue;}
...
/*------------------------------------------------------------------------*/
shint podzial_na_slowa(char *text,char *slowa[],shint max_l_slow,char f,char male)
{shint nr_slowa=0;
 char *sl;
 do
...
char znaki_liczby(char *slowo,char kropka)
 {char kr,e,z;
  ...
  do{z=0;
     ...
     if(*slowo<58 && *slowo>47)  {slowo++; z=1; continue;}
     if(kr>0 && *slowo=='.') {slowo++; kr=0; z=1; continue;}
     if(e==2 && *slowo=='e')
      {slowo++; e=0;
       if(*slowo=='+' || *slowo=='-') slowo++;
      }
...
char *szukaj_slowa(char *text,char f)
 {register i;
   shint lsep;
   lsep=L_SEP; if(f>0) lsep=lsep-f;
   for(;;)
    {for(i=0;i<lsep;i++) {if(*text==separator[i]) goto S;}
     return text;
S:   if(*text==EOS) return text;
		 text++;
    }
 }  /*....end szukaj_slowa()....*/
/*------------------------------------------------------------------------*/
...
void mAbort()
{yplus(); x_akt=x_akt0;
 term_printf(y_akt,x_akt,attryb,"ÉÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ»");
 yplus(); x_akt=x_akt0;
 term_printf(y_akt,x_akt,attryb,"º   ABORT programu !!! ?? TAK - 't'     º");
 yplus(); x_akt=x_akt0;
 term_printf(y_akt,x_akt,attryb,"ÈÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼");
 yplus(); x_akt=x_akt0;
...
void bip(char mod)
 {
#ifndef _TERM_QNX_
 if(mod==0) {sound(500); delay(100);}
 else {sound(1000); delay(200);}
 nosound();
#endif
 }
/* -----------------------------------------------------------------------*/
void yplus(void) {if(y_akt<y_maxOkna) y_akt++;  return;}
/* -----------------------------------------------------------------------*/
void xplus(void) {if(x_akt<x_maxOkna) x_akt++;  return;}
...
#ifndef _CVC_
shint Get_char(void){return getch();}
#else

shint Get_char(void){return term_new_getch_char(-1);}
#endif


void zamknij_grafiki(shint a) {};
...
/* shint GETchar(void){return getchar();} */
shint Get_Char(void){return getchar();}
#define L_EVENT 2
#define SPEC1 255
shint GET_char(void)  /* ======= odczyt znaku z myszki lub klawiatury === */
 {static shint znak=-2;
  static shint buf_znak=-2;
  shint siz;
...
/* fprintf(mystderr," RETURN %d/%c",znak,znak); */
  ...
  do
     ...
     {M_ev=BUF_com; siz=BUF_com_size;}
		else {M_ev=&Mouse_ev[0];
          siz=sizeof(struct sd1); if(siz<M_BUF_SIZ) siz=M_BUF_SIZ;
         }
    buf_adr=(char **)M_ev; buf_adr[0]=NULL; buf_adr[1]=NULL;
...
/* fprintf(mystderr," l_events=%d",l_events); */
           ...
           {if(M_ev->dy>6) buf_znak=UP;
            ...
            if(buf_znak>-2) break;
            if(M_ev->dx<-10) {buf_znak=LEFT; znak=BS; buf_znak=0;}
            if(M_ev->dx>10) buf_znak=FFR;
            if(buf_znak>-2) break;
            if(M_ev->buttons>0) {znak=CR; buf_znak=0;}
            if(buf_znak>-2) break;
					 }
          if(buf_znak>-2) break;
				 }
...
/* fprintf(mystderr,"\nDEV: i=%d znak=%c/%d buf_znak=%d armed=%d wait_syn=%d",i,znak,znak,buf_znak,armed,wait_for_synch_trig); */
          ...
          if((znak==SPEC || znak==SPEC1) && buf_znak==-2) {armed=0; continue;}
         }
       }
      else
       {znak=sygnal_grafiki(w_pid,&Mouse_ev[0]);
				if(znak>0) {buf_znak=-2; znak=-2;}     // obsluzono grafiki - dalej czekac
				else
         {if(odbior_komunik!=NULL)
					 {nr_komunik=odbior_komunik(BUF_com,w_pid); //GetMessage(w_pid,(char *)buf);
/* fprintf(mystderr,"\nwewy.c Po: nr_komunik=%d lwmall=%d",nr_komunik,lwmall); */
            if(nr_komunik>0)
             {if(nr_komunik==CL_READY) wait_for_synch_trig=0; znak=0; continue;}
            if(nr_komunik==TIMEOUT)
						 {komunikat(MY_MAX,X_L0,ATTR_A," Brak zadania synchronizowanego z klawiatura !!! <q - exit> ");
              if(getchar()=='q') exit(1); else  {wait_for_synch_trig=0; znak=-1; break;}
             }
            else
						 {if(nr_komunik==COM_PROGR) {/*fprintf(mystderr," zwoln.");*/ znak=0; continue;}
							Reply(w_pid,&BUF_com,sizeof(void *)); wait_for_synch_trig=0;
              znak=-1; break; //continue;
						 }
					 }
         }
       }
...
shint m_wherex(void) {return term_state.col;}
shint m_wherey(void) {return term_state.row;}
...
shint update_wewy_grafs(void *RGn, void *RGs, shint raport,
   ...
   {for(i=0;i<BGR.Lgrafs;i++) {if(BGR.Grafiki[i]==RGs) break;}
    if(i<BGR.Lgrafs)
     {BGR.report[i]=-1;
      BGR.Lgrafs--;
      for(;i<BGR.Lgrafs;i++)
       {BGR.Grafiki[i]=BGR.Grafiki[i+1]; BGR.report[i]=BGR.report[i+1];}
     }
    return BGR.Lgrafs;
	 }
  if(BGR.akcja_grafik!=NULL)
   ...
   {BGR.Grafiki[BGR.Lgrafs]=RGn; BGR.report[BGR.Lgrafs]=raport;
...
/* fprintf(mystderr," Gafiki[%d]=%d",i,BGR.Grafiki[i]); */
     {if(report<0 || BGR.report[i]==report)
       {if(BGR.akcja_grafik!=NULL)
        ...
        BGR.Grafiki[i]=NULL; BGR.report[i]=-1;
				BGR.Lgrafs--;
			 }
     }
   }
/* fprintf(mystderr,"\n wewy: Koniec Lgrafs=%d akcja=%d",BGR.Lgrafs, BGR.akcja_grafik); */
  if(BGR.Lgrafs<=0) {BGR.Lgrafs=0; BGR.akcja_grafik=NULL; BGR.Grafiki[0]=NULL;}
  return;
 }
...
*/
	if(lenth==0) {term_type(y,x,text,0,attr); lenth=strlen(text);}
	else {if(lenth==1) term_type(y,x,&text[0],1,attr);
				else {l=strlen(text);
...
{unsigned int att;
 shint k_inv, i, y, dl_tyt, l_linii, k, tx_size, ret, ip, k_ost;
 shint xp, xm, yp, ym, Ym, y_kur, x_kur, n, ik, x, ip_old;
 char znak, **buf, *text, *koniec="<Esc>", jest;
	if(Xp<X_L0) Xp=X_L0; if(Xm>MXR_MAX) Xm=MXR_MAX;
  if(Yp<Y_G0) Yp=Y_G0;
	tx_size=Xm-Xp;
	dl_tyt=0;
	if(tytul!=NULL) dl_tyt=strlen(tytul);
	if(tx_size<dl_tyt)
	 {for(i=0; i<kmax;i++) {y=strlen(opcje[i]); if(y>tx_size) tx_size=y;}
		k=strlen(koniec); if(tx_size<k) tx_size=k;
		if(tx_size<dl_tyt) tx_size=dl_tyt;
		tx_size+=2; if(ramka>0) tx_size+=2;
		if(tx_size>MXR_MAX)
	...
	if(Xm>MXR_MAX)
	 {i=Xm-MXR_MAX; Xp-=i; Xm=MXR_MAX;
		if(Xp<X_L0) {tx_size-=Xp; Xm-=Xp; Xp=X_L0;}
	 }
	xp=Xp; yp=Yp; xm=Xm;
	if(ramka>0) {xp++; xm--;}
	Ym=Yp+kmax; ym=Ym;
	if(dl_tyt>0) {Ym++; yp++; ym++;}
	if(ramka>0 && dl_tyt>0) Ym++;
	if(dl_tyt==0 && ramka>0) {yp++; ym++; Ym+=2;}
	if(Ym>MYR_MAX) {Ym=MYR_MAX; ym=MYR_MAX; if(ramka>0) ym=MYR_MAX-1;}
	l_linii=Ym-Yp+1;
	blok_anim->xp=Xp; blok_anim->yp=Yp;
	blok_anim->xm=Xm; blok_anim->ym=Ym;
	...
	lwmall++;
	attr=attr&(~TERM_FLUSH); attr_o=attr_o&(~TERM_FLUSH);
	tx_size++;
	k=term_save_image(yp,xp,"a",0);
	for(y=Yp;y<=Ym;y++)
	 {n=y-Yp; buf[n]=(char *)Malloc(tx_size*k+1);
		if(buf[n]==NULL)
		 {komun_text(MY_MAX,1," Brak pamieci dla menu. <Ent>",0,ATTR_A);
			for(k=0;k<n;k++) {Free(buf[k]);lwmall--;} Free(buf); lwmall--;
			GET_char(); return -1;
		 }
		lwmall++;
...
/*fprintf(mystderr,"\nokno_menu(przed Okno): Yp,Xp,Ym,Xm=%d %d %d %d; x_maxOkna=%d y_maxOkna=%d; X_L0,Y_G0,MXR_MAX,MY_MAX=%d %d %d %d",Yp,Xp,Ym,Xm,x_maxOkna,y_maxOkna,X_L0,Y_G0,MXR_MAX,MY_MAX); */
	...
	if(dl_tyt>0) {i=xp+(xm-xp+1-dl_tyt)/2; term_type(Yp,i,tytul,0,attr);}
	if(opcja0<0) opcja0=0;
	if(kmax>0) opcja0=opcja0%kmax; else opcja0=0;
	ip=0; k_inv=opcja0; l_linii=ym-yp; ip_old=-1; jest=0; k_ost=k_inv;
	ik=ip+l_linii;
	...
	do
	 {ik=ip+l_linii; y=yp;
		for(i=ip; (i<=ik)||(jest!=0); i++, y++)
		 {if(ip==ip_old)
			 {switch(jest)
				 {case 0: i=k_ost; y=yp+i-ip; jest=1; break;
					case 1: i=k_inv; y=yp+i-ip; jest=2; break;
					case 2: jest=0; goto W;
				 }
			 }
			att=attr; if(i==k_inv) {att=attr_o; x_kur=xp+2; y_kur=y;}
			if(i<kmax) text=opcje[i]; else text=koniec;
			znak=EOS;
			k=strlen(text); if(k+xp>xm-1) {znak=text[xm-xp-1]; text[xm-xp-1]=EOS;}
			term_type(y,xp," ",1,att); term_type(y,xp+1,text,0,att);
...
W:  do
P:   {term_cur(y_kur,x_kur);
			k_ost=k_inv;
			term_flush();
			
			ret=GET_char(); ret=ret&0x00ff;
			
			switch(ret)
			 {case NLCR: ret=CR/*Get_Char()*/;
				case CR:  if(k_inv==kmax) ret=Esc; goto D;
				case Esc: ret=Esc; goto D;
				case BS:  k_inv--; /*if(k_inv<0) k_inv=0;*/ break;
				case SPEC:
				 ret=GETchar();	   ret=ret&0x00ff; /*  printf(" drugi=%d",ret); */
				 switch(ret)
					{case Home: k_inv=0;    break;
					 case End:  k_inv=kmax; break;
					 case PgUp: k_inv-=5;   break;
					 case PgDn: k_inv+=5;   break;
					 case LEFT:
					 case UP:   k_inv--;    break;
					 case FFR:
					 case DOWN: k_inv++;    break;
					 default: goto P;
					}
				 break;
				default: goto P;
...
/* ===================================================================== */
void maluj_ramke(shint yp, shint xp, shint ym, shint xm, unsigned int attr)
 {shint i;
	if(xp<X_L0) xp=X_L0; if(yp<Y_G0) yp=Y_G0;
	if(xm>MXR_MAX) xm=MXR_MAX; if(ym>MYR_MAX) ym=MYR_MAX;
#ifdef GRAF
	 if(igraf!=0) ramka_graficzna(yp, xp, ym, xm, attr);
	 else
#endif
		{term_type(yp,xp,"É",1,attr);
		 for(i=xp+1;i<xm;i++) term_type(yp,i,"Í",1,attr);
		 term_type(yp,xm,"»",1,attr);
		 for(i=yp+1;i<ym;i++)
			{term_type(i,xp,"º",1,attr); term_type(i,xm,"º",1,attr);}
		 term_type(ym,xp,"È",1,attr);
		 for(i=xp+1;i<xm;i++) term_type(ym,i,"Í",1,attr);
...
void Okno(shint y0, shint x0, shint ymax, shint xmax, unsigned int attr)
 {if(x0<X_L0) x0=X_L0; if(x0>=(MXR_MAX)) x0=MXR_MAX-1;
	if(y0<Y_G0) y0=Y_G0; if(y0>MY_MAX) y0=MY_MAX;
	if(xmax<X_L0) xmax=X_L0; if(xmax>MXR_MAX) xmax=MXR_MAX;
	if(ymax<Y_G0) ymax=Y_G0; if(ymax>MY_MAX) ymax=MY_MAX;
	x_akt0=x0; x_akt=x0; y_akt=y0; y_akt0=y0; y_maxOkna=ymax; x_maxOkna=xmax;
	x_min=x0; y_min=y0;
	attryb=attr;
...
shint komunikat(shint y, shint x, unsigned int attr, char *format,...)
 {shint l, i=0;
	...
	va_end(arg);
	l=strlen(bufor); if(x+l>=MX_MAX) bufor[MX_MAX-x-l]=0;
	for(i=x+l;i<MX_MAX;i++) term_type(y,i," ",1,attr);
	term_type(y,x,bufor,0,attr); term_color(TERM_WHITE|TERM_BLACK_BG);
	term_type(y,MXR_MAX," ", 1,TERM_WHITE|TERM_BLACK_BG);
	return m_wherex();
 }

shint komun_text(shint y, shint x, char *bufor, shint lz, unsigned int attr)
 {shint l, i=0;
	l=strlen(bufor); if(x+l>=MX_MAX) bufor[MX_MAX-x-l]=0;
	for(i=x+l;i<MX_MAX;i++) term_type(y,i," ",1,attr);
...
void wewy_abort_off(void) {ABORT=0; BLAD=0;}
void wewy_abort_on(void) {ABORT=1; BLAD=1;}
...
void ustaw_wybor_na_danej(shint nr)
 {if(nr<0) nr=0; dana_poczatkowa=nr;}
void przepisz(char *do_ob, char *z_ob, shint ile)
  {char *adr_max;
   adr_max=z_ob+ile;
  ...
  shint i=0, zn;
   ...
   do
    {z=szukaj(z,'%');
     if(z!=NULL)
			{zn=czy_format(z,&i);
       if(zn==EOS) {z++; continue;}
       if(typ!=zn)
				{komunikat(MY_MAX,1,ATTR_A," Bledny format %s w okno_tabl_..(). <Ent> ",
                     format);
         GET_char(); z=NULL; break;
        }
       if(k==0) *zp=z;
       c=z+i; k++; z=c;  /* szukamy nowego formatu od adresu z */
      }
    }while(z!=NULL && k<2);
  return z;
 }
...
shint save_var(shint buf[])
 {shint siz=0;
	if(buf==NULL) return 31+sizeof(struct Blok_anim)/sizeof(shint)+1;
	buf[0]=X_0; buf[1]=Y_0; buf[2]=X_maxBlank; buf[3]=Y_maxBlank; buf[4]=x_maxOkna;
	buf[5]=y_maxOkna; buf[6]=x_akt; buf[7]=y_akt; buf[8]=x_akt0; buf[9]=y_akt0;
	buf[10]=x_min; buf[11]=y_min;
	buf[12]=Uprawn_max; buf[13]=Uprawn_pocz;
	buf[14]=bez_potwierdzania; buf[15]=ZAPIS;
	buf[16]=blokada_potwierdzania; buf[17]=Uprawn_max; buf[18]=Uprawn_pocz;
	buf[19]=bez_potwierdzania; buf[20]=blokada_potwierdzania;
	buf[21]=BEZ_HASLA; buf[22]=EDIT; buf[23]=SLEDZIC;
	buf[24]=blokada_zapisu; buf[25]=dana_poczatkowa;
	buf[26]=attryb;
	buf[27]=m_wherex(); buf[28]=m_wherey();
	buf[29]=xg_akt; buf[30]=yg_akt;
	siz=sizeof(struct Blok_anim);
	przepisz((char *)&buf[31], (char *)blok_anim,siz);
	siz=31+siz/sizeof(shint)+1;
	return siz;
 }
...
shint restore_var(shint buf[])
 {shint siz;
	...
	X_0=buf[0]; Y_0=buf[1]; X_maxBlank=buf[2]; Y_maxBlank=buf[3]; x_maxOkna=buf[4];
	y_maxOkna=buf[5]; x_akt=buf[6]; y_akt=buf[7]; x_akt0=buf[8]; y_akt0=buf[9];
	x_min=buf[10]; y_min=buf[11];
	Uprawn_max=buf[12]; Uprawn_pocz=buf[13];
  bez_potwierdzania=buf[14]; ZAPIS=buf[15];
	blokada_potwierdzania=buf[16]; Uprawn_max=buf[17]; Uprawn_pocz=buf[18];
	bez_potwierdzania=buf[19]; blokada_potwierdzania=buf[20];
	BEZ_HASLA=buf[21]; EDIT=buf[22]; SLEDZIC=buf[23];
	blokada_zapisu=buf[24]; dana_poczatkowa=buf[25];  attryb=buf[26];
	term_cur(buf[28], buf[27]);
	xg_akt=buf[29]; yg_akt=buf[30];
	siz=sizeof(struct Blok_anim);
	przepisz((char *)blok_anim, (char *)&buf[31], siz);
	return 31+siz/sizeof(shint)+1;
 }
...
/*  delay(800); */
 blok_anim->ym=-1;
	Uprawn_max=0; Uprawn_pocz=0; Kod_operatora_max=NULL;
	bez_potwierdzania=0; blokada_potwierdzania=0;
	X_0=X_L0; Y_0=Y_G0; X_maxBlank=MXR_MAX; Y_maxBlank=MY_MAX;
	x_maxOkna=MXR_MAX; y_maxOkna=MY_MAX; x_akt=X_0; y_akt=Y_0; x_akt0=X_0; y_akt0=Y_0;
 x_min=X_0; y_min=Y_0;
	wewy_abort_off();
	dana_poczatkowa=0; SCROLL=-1;
	arrow=27;
	if(BUF_com==NULL || BUF_com_size==0 || odbior_komunik==NULL)
	 {BUF_com=NULL; BUF_com_size=0; if(real_time==0) odbior_komunik=NULL;}
#ifndef _DOS_
	else
...
/*  ......  if(_TYP_KONSOLI_==2) Mouse_ctrl=NULL; */
	 if(kproxy<0) {open_konsole_mouse(0); if(kproxy>0) mouse_free=1;}
	if(real_time==1) {BUF_com=&Mouse_ev[0]; BUF_com_size=M_BUF_SIZ;}
	wait_for_synch_trig=0;
	BGR.Lgrafs=0; BGR.akcja_grafik=NULL; BGR.Grafiki[0]=NULL; BGR.report[0]=-1;
#endif
	lhasel=0;
	ZAPIS=0; BEZ_HASLA=0; EDIT=0; SLEDZIC=0;  blokada_zapisu=ZAPIS_DOWOLNY;
	if(Klucz!=-1)
	 {komunikat(MY_MAX,X_L0,ATTR_A," Nie zamkniete raporty poprzedniej sesji    <Ent>    ");
		GET_char(); return -1;
	 }
	for(i=0;i<LHASEL;i++) {haslo[i][0]=EOS; kod_o[i][0]=EOS;}
	if(lwmall>0)
	...
	if(nr_blank>=0 || Czy_Blankiet_otwarty!=0)
	 {komunikat(MY_MAX,1,ATTR_A,"Nie zamkniete blankiety (ostatni %d, stan %d) z poprzedniej sesji: <q> - koniec; <d> - dalej ?? ",
														 nr_blank, Czy_Blankiet_otwarty);
		do{zn=GET_char();  zn=zn&0x00ff;}while(zn!='q' && zn!='d');
 		if(zn=='q') return -1;
		do{i=BL[nr_blank]->nr_rekordu; zamknij_blankiet(&i);}while(nr_blank>=0);
	 }
	Czy_Blankiet_otwarty=0; nr_blank=-1;
	for(i=0;i<=LRAP_BL;i++)
	 {RAP[i]=NULL;  
		if(RAP[i]!=NULL)
		 {komunikat(MY_MAX,1,ATTR_A,"Nie zamkniety raport %d z poprzedniej sesji: <q> - koniec; <d> - dalej ?? ",i);
			do{zn=GET_char();  zn=zn&0x00ff;}while(zn!='q' && zn!='d');
			if(zn=='q') return -1;
		 }
	 }
	for(i=0;i<LBLANK;i++) BL[i]=NULL;
	NR_OPER=nr_op; Klucz=0;
#ifdef _DOS_
#ifndef GRAF
	window(1,1,80,25);
#else
     ...
     i=0;
		 if(BL[nr_blank]==NULL)
			{komunikat(5,1,ATTR_A," Zla specyfikacja blankietu %d: <Ent>", nr_blank);
			 nr_blank--; i=1; k=1; GET_char();
      }
    }while(i==1);
	 Okn=BL[nr_blank]->Okbl; if(Ok==NULL) Ok=Okn;
   if(Ok!=Okn)
		{komunikat(5,1,ATTR_A," Blankiet %d Ok=%d: <Ent>", nr_blank,Ok);
		 ok=Ok;
		 for(i=0;i<liczba_danych;i++,ok++)
      {if(ok->typ=='k' && ok->format!=NULL) {Free(ok->format); lwmall--;}
      }
     Free(Ok); lwmall--; GET_char();
    }
   ...
   {ok=Ok;
    ...
    " Niezgodnosc rekordu dekl.(%d) z zapisem (%d) w zamykanym blankiecie %d. <ENT> ",
      ...
      GET_char();
		...
		if(nr_blank==0)
     {}
		Free(BL[nr_blank]); BL[nr_blank]=NULL; lwmall--;
    nr_blank--;
		nr_danej=0; X_0=X_L0; Y_0=Y_G0; X_maxBlank=MXR_MAX; Y_maxBlank=MY_MAX; HELP=0;
   }
  if(nr_blank>=0)
   {Czy_Blankiet_otwarty=2;
    Ok=BL[nr_blank]->Okbl;
...
/* --------------------- KONIEC MACRA ----------------------------- */
char ustaw_buf_wpisu(shint y, shint x, char *monit, shint *size,
										 unsigned int attr_monit, unsigned int attr_wpis)
 {shint dl, dlug, x0;
	char znak=0;
	wewy_abort_off(); INSERT=1;
	if(attr_monit==0) attr_monit=attryb;
	if(attr_wpis==0) attr_wpis=attryb;
	if(y<0) y=m_wherey(); if(x<0) x=m_wherex();
	if(y<Y_G0) y=Y_G0; if(x<X_L0) x=X_L0;
	if(y>MY_MAX) y=MY_MAX; if(x>MX_MAX) x=MX_MAX;
	dlug=strlen(monit); if((*size)<1) (*size)=1;
	dl=MX_MAX-dlug-(*size);
	if(dl<0) dlug-=dl; znak=monit[dlug]; monit[dlug]=znak;
	if((dlug+(*size)+x)>MX_MAX)
	 {if(y==MY_MAX) x=X_L0;
		else {y++; x=X_L0;}
	 }
	if(dlug>0)
	 {term_type(y,x, monit, dlug ,attr_monit); x=m_wherex();}
	else {term_cur(y,x);}
...
shint wczytaj_int(shint y, shint x, char *monit, shint size, shint w_def,
 ...
 {shint x0, ret, i;
	...
	attr_old=attryb;
	if(size>=WLMAX) size=WLMAX-1;
	if(w_def<w_min) {w_def=w_min; oblig=1;}
	if(w_def>w_max) {w_def=w_max; oblig=1;}
	liczba=w_def;
	if(monit[0]=='#') oblig=1;
...
float wczytaj_float(shint y, shint x, char *monit, shint size, shint prec,
 ...
 {shint x0, ret, i;
	...
	x0=ustaw_buf_wpisu(y, x, monit, &size, attr_monit, attr_wpis);
	y=m_wherey();
	if(prec>=size) prec=size-1; if(prec<0) prec=0;
...
/*	if(strl==0)
	...
	if(strl>size) {text[size]=EOS; strl=size; oblig=1;}
/*	macro_wpis("%c"); */
	if(oblig==0)
	 {sprintf(buf,"%s",text);
		term_type(y,x0,buf,size,attryb); term_cur(y,x0);
	 }
	else {buf[0]=EOS; if(size>1) INSERT=-1;}
	while(1)
...
/* == bscanf zwaraca: -2 <Esc>; -1 <Ent>; 0 pusty; >0 l.znakow w buf === */
		if(ret==-1 && oblig==1)     /* <Ent> */
		 {term_type(y,x0-1,"?",1,ATTR_A); term_cur(y,x0); continue;}
		if(ret<-1)
		 {attryb=attr_old; term_color(attryb);	term_cur(y,x0); return 0;} /* <Esc> */
...
shint monit_textowy(shint yp, shint xp, unsigned int attr, char *text)
 {shint k, tx_size, ret, dl;
	char *buf, znak=0;
	shint xkursora, ykursora;
	unsigned int old_attr;
	xkursora=m_wherex(); ykursora=m_wherey();
	setcursor(nocursor);  old_attr=attryb;
	k=term_save_image(yp,xp,"a",0);
	tx_size=strlen(text); dl=MX_MAX-(xp+tx_size);
	if(dl<0) {tx_size+=dl; znak=text[tx_size]; text[tx_size]=EOS;}
	buf=(char *)Malloc(tx_size*k+1);
	if(buf!=NULL) {lwmall++;	term_save_image(yp,xp,buf,tx_size);}
...
#include "wewy1.h"
extern shint (*odbior_komunik)(void *b, pid_t a);
extern shint BUF_com_size, nr_komunik;
extern void *BUF_com;
extern char real_time;
#endif
/* =================================================================== */
...
shint objasn_grupy(shint y, shint x, shint w_min, shint w_max, shint wartn,
 ...
 {shint x_k_help;
   if(wartn<w_min || wartn>w_max)
     ...
     x_k_help=m_wherex()-1;
		 Okno(y,x,y,x_k_help,attr); clr_Okno();
     term_type(y,x," ",1,attrtx);
		 Term_Typef(y,x+1," ??? ",5,attr);
		}
	 else {disp_help(y,x,attrtx,attr,wartn,skok,R,help, X_maxBlank-dltn+1);
         x_k_help=m_wherex();
        }
/*fprintf(mystderr,"\nPo grupie"); */
   return x_k_help;
...
/* ------------------------------------------------------------------- */
shint wydruk_monitu(int wpis, char *opis_danej, shint dl_zakr, char *taknie,
 ...
 {char *fmon;
  shint ret, i, ms, xmax, k, l;
   *dltn=0;
   if(bez_potwierdzania==0) *dltn=strlen(taknie);
   i=dl_zakr; if(i>0) i--;
   fmon=&opis_danej[i];
   ms=strlen(fmon);  text[0]=EOS;
   for(i=ms-2;i>=0;i--)
    {if(fmon[i+1]=='?' && fmon[i]==' ')
      {for(k=i;k<=ms;k++) text[k-i]=fmon[k];
       fmon[i+1]=EOS; fmon[i]=EOS;
       break;
      }
    }
   ...
   *xp=*xp_naz+dx_ind;
   ...
   for(*txl=0; *txl<MXR_MAX;(*txl)++) {if(text[*txl]==EOS) break;}
	  ret=xmax+4; if(wpis!=2) ret=xmax+(*dltn); k=0;
	  if(help!=NULL)
		  {if(typ!='g')
			   {if(typ=='z') {w_min=0; w_max=skok-1;}
			    if(typ=='i' && w_max>skok-1) w_max=skok-1;
			    for(i=w_min;i<=w_max;i++) {l=strlen(help[i]); if(l>k) k=l;}
			    if(typ=='z') {k--; if(k<6) k=6;}
			   }
     *dl_menu=k;
    }
   k=ret+(*dl_menu);
	 if(k>X_maxBlank)
		{if(y<=Y_maxBlank && y<MY_MAX) return 0;
		 (*x0)+=(X_maxBlank-k);
     if((*x0)<X_L0 && ms>=-(*x0)) {opis_danej[ms-(*x0)]=EOS; (*x0)=X_L0;}
     if((*x0)<X_L0 && ms<-(*x0))  {opis_danej[0]=EOS; (*x0)=X_L0;}
     *xp_naz=(*x0)+ms;
     *xp=*xp_naz+dx_ind;
     xmax=(*xp)+(*size)-1;
    }
...
shint wpis_indexu(shint y, shint xp_ind, shint ind_size, shint *wart,
                  char *index_helpu, shint size_helpu, shint w_min,
                  shint w_max, char *adr, shint dim, shint xp,
                  shint xmax, char *f_ind, char *format, unsigned int attrtx,
                  unsigned int attr, long int skok, void *R, void *Rh, 
                  char **help, char typ, shint dltn, shint W_min, shint W_max, shint lsmax)
 {char form[6], bufor[30], c_tab, *zle, *menu, ogr, err=0;
  shint ret, wartn, l_tab, i, xk, nr_zm, ins, Ww_max, l_help;
  float w_tab;
     zle=" ?? ";
     wartn=*wart;
     sprintf(bufor,f_ind,wartn);
     sprintf(form," d"); form[0]='%';
     xk=xp_ind+ind_size-1;
     nr_zm=*wart;
     do
      {ins=INSERT; INSERT=1;  HELP=1;
			 Okno(y,xp_ind,y,xk,attr);
       nr_zm=wartn;
       ret=bscanf(bufor,attrtx,form,&wartn);
       INSERT=ins;
       if(ret<0)
        {HELP=0; if(wartn<w_min) wartn=w_min; if(wartn>w_max) wartn=w_max;
         *wart=wartn; term_printf(y,xp_ind,attr,f_ind,wartn);
        }
       if(ret==0) HELP=0;
       ogr=0;
       if(wartn<w_min)
        {wartn=w_min; term_printf(y,xp_ind,attr,f_ind,wartn);
         ogr=1;
        }
       if(wartn>w_max)
        {wartn=w_max; term_printf(y,xp_ind,attr,f_ind,wartn);
         ogr=1;
        }
...
/*              lsmax=0;
            ...
            break;
           case 'g':
            l_tab=*(shint*)(adr+wartn*dim); Ww_max=W_max; l_help=l_tab;
            err=0; 
            if(size_helpu>0)
						       {long int skok_sum;
       							skok_sum=ustal_skok_adr(Rh, l_tab, size_helpu, &err);
							       if(err==0)
       							 l_help=*(shint*)((unsigned long int)index_helpu+skok_sum);
							       Ww_max=9999;
						       }
            term_printf(y,xp,attr,format,l_tab);
           ...
           case 'z':
            ...
            if(help==NULL) term_type(y,xp,&c_tab,1,attr);
             ...
             {menu=zle; /*lsmax=0; */
              for(i=0;i<skok;i++)
               {/*ls=strlen(help[i]); if(ls>lsmax) lsmax=ls; */
                if(c_tab==help[i][0]) {menu=&help[i][2]; break;}
               }
							i=lsmax+xp; if(i>X_maxBlank) lsmax=(X_maxBlank-xp);
...
char *formaty_tabl(char *f_nazwy, char *monit_pocz, shint *ind_min,
 ...
 {char *fmon, *z, *c, im, zn, *fp, *f_pocz;
  shint i, j, k, l;
   fmon=monit_pocz; im=0;
   if(monit_pocz[0]=='#' || monit_pocz[1]=='#') *hash='#';
   if(fmon[0]=='#' || fmon[0]=='*' || fmon[0]=='+') im=1;
   if(fmon[1]=='#' || fmon[1]=='*' || fmon[1]=='+') im=2;
   fmon+=im;
   *dl_zakr=0; opis_danej[0]=EOS;
   f_nazwy[0]=EOS;
/* ----------------- wstepne operacje dla tablicy -------------------- */
   if(*ind_max<=0 || *ind_max<=*ind_min) {*ind_min=0; *ind_size=0;}
   else
    {*ind_size=1; if(*ind_max>9) *ind_size=2; if(*ind_max>99) *ind_size=3;}
      ...
      {fp=format_tabl;      /* tj. f_nazwy, f_ind, fk_nazwy */
       ...
       do
        {for(z=fp;*z!='%' && *z!=EOS; z++,j++)
          {if(l<32) {f_nazwy[l]=format_tabl[j]; l++;}
           else {*z=EOS; break;}
          }
         if((*z)=='%')
          {zn=czy_format(z,&i);   /* prawdop. specyfikacja */
           if(zn==EOS)
            {fp=z+1; continue;}   /* nie jest to specyfik. */
           if(k==0)               /* znaleziono specyfikacje */
            {f_nazwy[l]=EOS; l++; *f_ind=&f_nazwy[l];
             j+=i;    /* przeskakujemy specyfikacje formatu */
             l+=4; (*f_ind)[3]=EOS;   /* zapelnimy format pozniej */
             *fk_nazwy=&f_nazwy[l];
             k++;
             fp=z+1; continue;
            }
           else                       /* druga specyfikacja */
            {if(czy_format(z,&i)==EOS) {fmon=z+1; continue;}
             k++; f_nazwy[l]=EOS;     /* konczymy format fk_nazwy */
            }
          ...
          {f_nazwy[l]=EOS; l++;
           if(k==0)
             ...
             *fk_nazwy=&f_nazwy[l]; *fk_nazwy[0]=' '; *fk_nazwy[1]=EOS;
            }
          }
        }while(k<2 && *z!=EOS);
      }
     else             /* brak formatu opisujacego tablice */
      {f_nazwy[0]=EOS; *f_ind=&f_nazwy[1];
       *fk_nazwy=*f_ind+4;
       (*fk_nazwy)[0]=' '; (*fk_nazwy)[1]=EOS;
      }
     sprintf(*f_ind," %dd",*ind_size); (*f_ind)[0]='%';
...
long int ustal_skok_adr(void *Rh, int nr, shint skok, char *err)
 {struct reports *R;
    ...
    {void *l, *next, *lp=NULL, *(*L);
     int k;
     L=(void **)(&(R->D));
     if((*L)==NULL) {*err=1; return 0;}
     l=(*L); next=*(void **)l;
     for(k=0; k<=nr; k++)
    		{if(next==NULL) break;
     	 lp=l; l=next; next=*(void **)l;
      }
     if(next==NULL && k<R->ob_konc) {*err=1; return 0;}
...
shint okno_help(shint Yp, shint Xp, shint Xm, unsigned int attr, unsigned int attr_o,
				...
				char *index_helpu, shint size_helpu, shint size, shint skok,
				void *R, void *Rh, char *help[], signed char ramka, char *tabl, shint dim,
				char *f_nazwy, char *f_ind, char *fk_nazwy)
{unsigned int att;
 shint k_inv, i, id, y, l_linii, k, tx_size, ret, ip, kmax, nr, k_ost, nrh;
 shint xp, xpf, xm, yp, ym, yd, Ym, y_kur, x_kur, n, ik, x, ind, ip_old, ind_size;
 char znak, **buf, *koniec="<Esc>", form_dat[10], *form_d, *menu, *zle, jest;
 shint dl_tyt=0, opcja0=0;
 float w, *wart;
 char err=0;
 struct dane_wpisu *dw;
	...
	zle=" ??? ";
	switch(typ)
	 {case 'f': typ=FLOAT; break;
		case 'z': typ=ZNAK;  break;
		case 'i': typ=INT;   break;
		case 'g': typ=GRUPA; break;
		default:  typ=INT;   break;
	 }
	if(size>0)
	 {form_d=&form_dat[0]; form_d[0]='%';
		sprintf(&form_d[1],"%dd",size);
		if(typ==FLOAT) sprintf(&form_d[1],"%d.%df",size,skok);
		ind_size=1; if(w_max>9) ind_size=2; if(w_max>99) ind_size=3;
	 }
	else {form_d=NULL; ind_size=0;}
	if(Xp<X_L0) Xp=X_L0; if(Xm>MXR_MAX) Xm=MXR_MAX;
	if(Yp<Y_G0) Yp=Y_G0;
	xp=Xp; yp=Yp; xm=Xm;
	if(ramka>0) {xp++; xm--;}
	 ...
	 {struct reports *Rd;
			...
			if(Rd->kod_raportu<0) /* jest to menu_rekordow */
			 {dl_tyt=strlen(Rd->tytul);
				if(dl_tyt>0) {i=xp+(xm-xp+1-dl_tyt)/2; term_type(Yp,i,Rd->tytul,0,attr);}
				opcja0=Rd->ob_pocz;
				if(opcja0<0) opcja0=0;
				if(w_max>0) opcja0=opcja0%w_max; else opcja0=0;
			 }
		}
	ip=0; k_inv=ip; ip_old=-1; l_linii=ym-yp, jest=0; k_ost=k_inv;
	yd=yp; id=ip;
	do
	 {ik=ip+l_linii; y=yp;
		for(i=ip; (i<=ik)||(jest!=0); i++, y++)
		 {if(ip==ip_old)
			 {switch(jest)
				 {case 0: i=k_ost; y=yp+i-ip; jest=1; break;
					 case 1: i=k_inv; y=yp+i-ip; jest=2; yd=y; id=i; break;
					 case 2: jest=0; goto W;
					}
			 }
			att=attr; if(i==k_inv) {att=attr_o; y_kur=y;}
			nr=i+w_min;
       ...
       {x_kur=x+size-1;
       	if(tabl!=NULL)
       	 {ind=nr;
       	  ...
       	  if(typ>ZNAK)   /* liczby calkowite */
     	     nr=*(shint *)(tabl+ind*dim);
					if(typ==FLOAT)
       	   {shint xpp, kol;
       	    for(kol=0,xpp=x;;)
       	     {w=*(float *)((tabl+(w_max+1)*dim*kol)+ind*dim);
	             if(form_d!=NULL) term_printf(y,xpp,att,form_d,w);
              kol++; if(kol==size_helpu) break;
	             xpp=m_wherex(); term_type(y,xpp," ",1,attr); xpp++;
	            }
            xpf=x;
       	    goto R;
       	   }
       	  if(typ==ZNAK)    /* znaki */
       	    ...
       	    if(help!=NULL)
       	     {for(k=0; k<skok;k++)
       	       {if(help[k][0]==nr) {menu=&help[k][2]; break;}}
       	      goto PISZ;
       	     }
       	    else goto R;
       	   }
       			 ...
       			 {long int skok_sum;
        				...
        				if(size_helpu>0)
       					 {skok_sum=ustal_skok_adr(Rh,nrh,size_helpu, &err);
       						 if(err==0)
         						nrh=*(shint*)((unsigned long int)index_helpu+skok_sum);
								 }
          ...
          if(help!=NULL && nr<skok) menu=help[nr];
       	 ...
       	 {if(nr<skok && help!=NULL)
       	   {term_type(y,x,&help[nr][0],1,att); menu=&help[nr][2];}
         	else term_type(y,x," ",1,att);
         }
        if(help!=NULL)
PISZ:    {term_type(y,m_wherex()," ",1,attr);
       ...
       {znak=EOS;
				k=strlen(koniec);
        if(k+xp>xm-1) {znak=koniec[xm-xp-1]; koniec[xm-xp-1]=EOS;}
        term_type(y,xp+1,koniec,0,att);
        if(znak!=EOS) koniec[xm-xp-1]=znak; att=attr;
       }
R:    for(x=m_wherex();x<xm;x++) term_type(y,x," ",1,att);
      term_type(y,x," ",1,attr);
     }
W:  ip_old=ip; ind=k_inv+w_min; id=k_inv;
    do
P:   {term_cur(y_kur,x_kur);
      k_ost=k_inv;
			term_flush();
      ret=GET_char(); ret=ret&0x00ff;/*printf("\n znak=%d",ret); */
      switch(ret)
       {case NLCR: ret=CR/*Get_Char()*/;
        case CR:
            ...
            {if(typ==FLOAT && dw!=NULL)
              {shint kol, index;
               ...
               if(dw->iobl==0) sprintf(dw->bufor,dw->format,*wart);
	              else dw->bufor[0]=EOS;
	   ...
	   goto D;
        ...
        case BS:  k_inv--; /*if(k_inv<0) k_inv=0;*/ break;
        case SPEC:
   	    ret=GETchar();  znak=znak&0x00ff; /*  printf(" drugi=%d",ret); */
SW:     switch(ret)
       	  {case Home: k_inv=0;    break;
					 case End:  k_inv=kmax; break;
           case PgUp: k_inv-=5;   break;
...
shint okno_tabl_znak(signed char wpis, shint y, shint x0,
 ...
 {shint ret, size, dlopt, txl, xp_ind, xp_naz;
  char text[80], znakp;
  char znk, znp, znf, sukces, *znak, hash='*';
  char *buffer, opis_danej[80], nznak, *taknie="<t/n>? ";
  char f_nazwy[40], *f_ind=NULL, *fk_nazwy=NULL, *fmon;
  shint xmax,xp, i, k, x_konc, zn, znw, xd, lopt, iopt;
  shint iobl, dltn=0, dx_ind=0, x, ind_size, dl_zakr;
  unsigned int att;
  char blok=0;
   if(wpis<-2) {blok=1; wpis=-wpis;}
   znak=(tabl+(*index)*dim);
   fmon=formaty_tabl(f_nazwy, monit_pocz, &ind_min, &ind_max, index,
               &ind_size, &hash, format_tabl, opis_danej, &dl_zakr, &f_ind,
               &fk_nazwy, &dx_ind);
   if(fmon!=NULL) sprintf(&opis_danej[dl_zakr],fmon,opcje);
   else sprintf(&opis_danej[dl_zakr],"[%s] ",opcje);
   lopt=liczba_obj;
   k=1; /* rozm_pola znaku */
   ...
   dltn=0;
   if(bez_potwierdzania==0) dltn=strlen(taknie);
   iobl=0; if(hash=='#') iobl=1;
   if(iobl==1 && wpis!=2) nznak='?';
   else nznak=*znak;
   znakp=nznak;
   iopt=0; xd=xp; dlopt=0;
   ...
   for(iopt=0;iopt<dlopt;iopt+=2)
    {znk=opcje[iopt];
     if(znk==nznak) break;
      ...
      {if(opcje[iopt+1]=='-')
        {znp=znk+1; znf=opcje[iopt+2];
         for(znk=znp;znk<znf;znk++) {if(znk==nznak) break;}
         if(znk<znf) break;
        }
      }
    }
   if(iopt>=dlopt) {iopt=0; znakp='?';}
   xp_ind=xp;
	 if(ind_size>0)
   ...
   xmax+=size;
OP:term_type(y,xp,&nznak,1,attr_d);  /* wydruk objasnienia znaku */
   sukces=0;
   xd=m_wherex()-1;
   if(size>0 && nznak!=' ')
    {term_type(y,m_wherex()," ",1,attrtx);
     for(i=0;i<liczba_obj;i++)
			{if(objasn[i][0]==EOS)
				{Term_Typef(y,m_wherex()," ??? ",size-1,attr); k=5; break;}
			 if(objasn[i][0]==nznak)
        {
		 ...
		 if(i==liczba_obj) {Term_Typef(y,m_wherex()," ??? ",size-1,attr); k=5;}
    }
   if(wpis==2)
    {for(i=0;i<txl;i++)
   ...
   if(ind_size>0 && wpis==-1)
    {i=*index;
     ret=wpis_indexu(y,xp_ind,ind_size,index,(char*)index,0,ind_min, ind_max, tabl,
		 ...
		 if(ret<0) {*index=i; return -5;}   /* tu bylo -1; */
     if(*index!=i)
      {znak=tabl+(*index)*dim;
       nznak=*znak;
      }
...
WP:do
		{do
			{/* m_gotoxy(xd,y); */
			 ...
			 term_cur(y,xp); term_flush();
			 zn=GET_char();  zn=zn&0x00ff;
			 if(zn==Esc)
				{if(sukces==0) {*znak=opcje[0]; *znak=znakp;}
				 return -x_konc;
				}
			 znw=zn; i=dlopt;
			 ...
			 if(zn==EOS || zn==SPEC)
				{zn=GETchar();  zn=zn&0x00ff;
				 if(zn==UP)
					{if(lopt>0)
						{if(opcje[iopt+1]=='-')
							{nznak++; if(nznak<opcje[iopt+2]) goto OP;} /* OP wydruk opcji */
						 iopt+=2; if(iopt>=dlopt) iopt=0;
						 nznak=opcje[iopt]; goto OP;
						}
					}
				 if(zn==DOWN)
					{if(lopt>0)
						{znk=nznak-1;
						 if(iopt>1 && opcje[iopt-1]=='-')
							{if(znk>opcje[iopt-2]) {nznak=znk; goto OP;}}
						 if(opcje[iopt+1]=='-')
							{if(znk>opcje[iopt]) {nznak=znk; goto OP;}}
						 iopt-=2; if(iopt<0) iopt=dlopt-1;
						 nznak=opcje[iopt]; goto OP;
						}
					}
				}
		 ...
		 if(objasn!=NULL && sukces!=2)
			{x=xp+2;
			 for(i=0;i<liczba_obj;i++)
				{if(objasn[i][0]==EOS)
					{Term_Typef(y,x," ??? ",size-1,attr); k=5; break;}
				 if(objasn[i][0]==nznak)
					{Term_Typef(y,x,&objasn[i][2],size-1,attr); break;}
				}
			}
		}while(znw!=CR);
	 if(*znak==nznak) return 0;
	 if(bez_potwierdzania==1) {znw='t'; goto TAK;}
	 if(xmax+dltn>X_maxBlank) {i=X_maxBlank-xmax; taknie[i]=EOS;}
   k=term_save_image(y,xmax+1,taknie,0);
   do{buffer=(char *)Malloc(dltn*k+1);
      if(buffer!=NULL) {term_save_image(y,xmax+1,buffer,dltn); lwmall++;}
      term_type(y,xmax+1,taknie,0,attrtx);
      i=m_wherex(); if(i<MXR_MAX) i--; i--; term_cur(y, i); term_flush();
      znw=GET_char();  znw=znw&0x00ff;
      if(buffer!=NULL)
       {term_restore_image(y,xmax+1,buffer,dltn); Free(buffer); lwmall--;}
      if(znw=='n') {wpis=1; nznak=zn; goto WP;}
      if(znw=='t')
TAK:   *znak=zn;
   ...
   for(i=0;i<dlopt;i+=2)
    {znk=opcje[i];
     if(znk==zn) break;
      ...
      {if(opcje[i+1]=='-')
        {znp=znk+1; znf=opcje[i+2];
         for(znk=znp;znk<znf;znk++) {if(znk==zn) break;}
         if(znk<znf) break;
        }
      }
		}
	 if(i<dlopt) {nznak=zn; *sukces=1; *iopt=i;}
	 return nznak;
	}
		 ...
		 {do{fm=szukaj(form,'%');
		   		if(fm==NULL)
				    {dlfor=strlen(form);
    					if(dlfor>0)
				    	 {xk=x+dlfor;
    						 if(xk>max_x) {i=max_x-x; k=format[i]; format[i]=EOS;}
...
/* fprintf(mystderr,"\nret.szukaj: format=%s %d",format,m_wherex()); */
			...
			nrzm++; if(nrzm==LHELP) break;
			fdanej=fm;
			dana=(char *)help[nrzm]; Dana=(unsigned long int)dana;
			if(typ=='s' || typ=='S')
			 {zn=form; if(*zn=='-' || *zn=='+') zn++;
 				if(*zn<'0' || *zn>'9') lenth=80;
	 			else {fm[koniec-1]=EOS; lenth=atoi(zn);}
		 		fm[koniec-1]='s';
			 }
...
/* ----- Dodano 19.08.1998 ----------------------------------------- */
      if(Skok<=0)                      /* menu z tablic param.objasniajacych */
       {switch(typ)
         {case 'S':
          case 's': skok=ind*sizeof(char*); typ='S'; break;
          case 'f': skok=ind*sizeof(float);  break;
          case 'i':
          case 'd': skok=ind*sizeof(shint);  break;
          case 'e': skok=ind*sizeof(double); break;
          case 'c': skok=ind*sizeof(char);   break;
          default:  break;
         }
       }
      shift=skok;
...
/* ----------------------------------------------------------------- */
      ...
      if(form[0]=='['&&form[1]=='%'&&form[2]=='a')
       {for(zn=&form[3],k=4;;zn++,k++)
         {if((*zn)==EOS) break;
          if((*zn)==']') break;
          if((*zn)<'0' || (*zn)>'9') {if(k>4) *zn=EOS; break;}
         }
...
/* fprintf(mystderr,"\n form=zn%d=%c %s (nrz=%d, help=%d)",k,*zn,form, nrz,help[nrz]); */
            if(nrz>=LHELP || help[nrz]==NULL)
						 {komunikat(MY_MAX,X_L0,ATTR_A," Brak liczby danych dla spec. %s w grupie - zn=%c k=%d. <Ent>", form,*zn,k);
              *zn=EOS; GET_char(); break;
             }
            else {l_opcji=*(shint*)help[nrz];  zn+=2; k=6;}
           }
          else
...
/* fprintf(mystderr, " nrzm=%d help=%d hel[nrzm]=%d shift=%d l_opcji=%d", nrzm,help,help[nrzm],shift,l_opcji); */
        ...
        if(shift<0 || shift>=l_opcji) shift=-1;  /* zly wskaznik */
         ...
         {switch(typ)
           {case 'S':
            case 's': shift=shift*sizeof(char*); typ='S'; break;
            case 'f': shift=shift*sizeof(float); break;
            case 'i':
            case 'd': shift=shift*sizeof(shint); break;
            case 'e': shift=shift*sizeof(double); break;
            case 'c': shift=shift*sizeof(char); break;
            default:  break;
           }
         }
       }
...
*/
            for(shift=0;shift<l_opcji;shift++,menu++)
             {if((*(char**)menu)==NULL) {shift=l_opcji; break;}
/*if(wzor=='z' || wzor=='Z') {komunikat(MY_MAX,m_wherex(),ATTR_A," %s% ",(*(char**)menu));  getchar();} */
          ...
          if(shift==l_opcji || dana==NULL)
           {
ERR_Z:      typ='S'; dana=(char*)brak; menu=NULL; shift=0;
           }
      ...
      *fdanej=EOS;  /*wyciecie formatu tekstu */
			...
			*fdanej='%';  /* rekonstrukcja oryginalnego formatu text+dana */
			if(k!=EOS)
			 {format[i]=k; *fm=znak; /*fprintf(mystderr,"\nret.k!=EOS");*/
				return x;
			 }
/* fprintf(mystderr,"\nTyp=%c: dana=%d shift=%d (dana+shift)=%d",typ,dana, shift,dana+shift); */
			if(shift>=0 || R!=NULL) dana=(char *)(Dana+shift);    /* wszystko OK !! */
			 ...
			 {shift=0;
     switch(typ)
         {case 'c': dana=f_bledu; f_bledu[0]='?'; break;
          case 'S':
          case 's': dana=(char*)brak; typ='S'; menu=NULL; shift=0; break;
          default:
            f_bledu[0]='%'; dana=(char*)brak;
            for(zn=&fdanej[1],k=1;k<4;zn++,k++)
             {if((*zn)<'0' || (*zn)>'9') {if(k>1 || (*zn)!='-') break;}
              f_bledu[k]=fdanej[k];
             }
            if(k==1) {f_bledu[1]='4'; k++;}
            f_bledu[k]='S'; f_bledu[k+1]=EOS;
            fdanej=f_bledu; typ='S';
            break;
...
/* -----if(typ=='c')
 ...
 ------------------------------------ */
      ...
      switch(typ)
       {case 'i':
        case 'd':
          sprintf(text,fdanej,*(shint*)(dana)); break;
        case 'f':
          sprintf(text,fdanej,*(float *)(dana)); break;
        case 'e':
          sprintf(text,fdanej,*(double *)(dana)); break;
        case 'c':
          sprintf(text,fdanej,*(dana)); break;
        case 'S':
          if(lenth>max_x) lenth=max_x;
					     if((*(char**)dana)==NULL)  {dana=(char*)brak; i=strlen(dana);}
...
/* fprintf(mystderr," text=%s###",text); */
          ...
          if(k!=EOS) {(*(char**)dana)[lenth]=k; k=EOS;}
          fdanej[koniec-1]='S';
          break;
        case 's':
          if(lenth>max_x) lenth=max_x;
...
*/
      ...
      i=strlen(text); xk=x+i;
      if(xk>max_x) {i=max_x-x; text[i]=EOS; i=0;}
      term_type(y,x,text,0,attr);
      x=m_wherex();
      format=form; *fm=znak;   /* wyciecie nastepnego formatu */
      if(i==0) return x;
...
shint wpisanie_danych_fl(shint x0, shint xp, shint y, shint xmax, shint x_konc,
 ...
 {float w_abs, wmin, wmax;
  shint xpp, nrkol, ins, ll, i, k, xa, ret, llmax, zn=' ';
  char *zakr[2], *buffer, zmiana=0; /* *fh, */
/* ## proba przerobki dla aktywnego help'u (brak w_min, w_max)
  --------------------------------------------------------------------- */
     HELP=0;
WPIS:xpp=xp; nrkol=0; zmiana=0;
     do
WPS:  {ins=INSERT; INSERT=1;
       do
        {Okno(y,xpp,y,xmax,attr);
         ...
         INSERT=ins;
         if(ret<0)
    	  {if(iffree==1) {Free(*format); *format=NULL; lwmall--; HELP=0;}
           return -x_konc;
          }
         if(HELP==2)
          {/* zmiana=1;  */
       ...
       HELP=0;
       zakr[0]=zakres; zakr[1]=zakr_abs;
       wmin=w_min; wmax=w_max; w_abs=wartn[nrkol];
       llmax=1; if(dlh_ogr>0) llmax=2;
       for(ll=0;ll<llmax;ll++)
        {if(w_abs<wmin || w_abs>wmax)
          {sprintf(text,*format,wartn[nrkol]); i=strlen(text);
           if(ll==1)
           ...
           if(zakr[ll][0]==EOS) sprintf(&text[i]," ZAKRES !! <ENT>-popraw ");
           else
            {if(ll==0) sprintf(&text[i],zakres," poza ",wmin,wmax," <ENT> ");
             if(ll==1) sprintf(&text[i]," poza%s <ENT> ", zakr_abs);
            }
					 i=strlen(text)+1; if(x0+i>X_maxBlank) {i=X_maxBlank-x0; text[i]=EOS;}
           xa=x_konc-i; if(xa<x0) xa=x0;
           ...
           if(buffer!=NULL)
            {lwmall++;
             term_save_image(y,xa,buffer,i);
						 term_type(y,xa," ",1,ATTR_A); term_type(y,xa+1,text,0,ATTR_A);
            }
           GET_char();
           if(buffer!=NULL)
	    {term_restore_image(y,xa,buffer,i); Free(buffer); lwmall--;}
					 wartn[nrkol]=*wart; goto WPS;
					}
         wmin=abs_min; wmax=abs_max; w_abs=wartn[nrkol]*baza;
         /* fh=format_ogr; */
        }
       if((*wart)!=wartn[nrkol]) {zmiana=1; wpisano_dana=1;}
/*       if(ret==0 && zmiana==0 || *wart==wartn[nrkol]) *wart=wartn[nrkol];
       else zmiana=1; */
       nrkol++;
       if(nrkol<L_kol)
        {wart=(float*)((tabl+(ind_max+1)*dim*nrkol)+index*dim);
				 if(iobl==0) sprintf(bufor,*format,wartn[nrkol]);
				 else bufor[0]=EOS;
         xpp=xmax+1; xmax=xpp+size; xpp++;
        }
      }while(nrkol<L_kol);
     if(zmiana==0)
      {if(iffree==1) {Free(*format); *format=NULL, lwmall--;}
       return 0; /*x_konc;*/
      }
     else ret=x_konc;
     xpp=xp;
       ...
       if(dlh_ogr>0)
        {term_type(y,x_konc,help_ogr,0,attrtx);
		 ...
		 if(bez_potwierdzania==1) {zn='t'; goto TAK;}
		 if(x_konc+dltn>=X_maxBlank) {i=X_maxBlank-x_konc; taknie[i]=EOS;}
     k=term_save_image(y,x_konc,taknie,0);
     do{buffer=(char *)Malloc(dltn*k+1);
        if(buffer!=NULL) {term_save_image(y,x_konc,buffer,dltn); lwmall++;}
        term_type(y,x_konc,taknie,0,attrtx);
        i=m_wherex(); if(i<MXR_MAX) i--; i--; term_cur(y, i); term_flush();
        zn=GET_char();  zn=zn&0x00ff;
        if(buffer!=NULL)
	 {term_restore_image(y,x_konc,buffer,dltn); Free(buffer); lwmall--;}
				if(zn=='n') {wpisano_dana=0; goto WPIS;}
				if(zn=='t')
         {
TAK:      for(i=0;i<L_kol;i++)
...
shint okno_text_menu(signed char wpis, shint y, shint x0, shint size,
 ...
 {shint ret, pznak;
  ...
  shint xmax,xp, ins, i, k, im=0, x_konc, zn, txl, dltn=0, dlstr;
  char blok=0;
	void *Rh=NULL; /*, *Rh=NULL; / * ???? */
	 if(wpis<-2) {blok=1; wpis=-wpis;}
	 dlstr=strlen(string); if(dlstr>size) {dlstr=size; string[dlstr]=EOS;}
	 if(bez_potwierdzania==0) dltn=strlen(taknie);
	 im=0;
	 if(monit[0]=='#' || monit[0]=='*' || monit[0]=='+') im=1;
	 if(monit[1]=='#' || monit[1]=='*' || monit[1]=='+') im=2;
	 ms=strlen(monit);  text[0]=EOS;
	 pznak=0;
	 for(i=ms-2;i>=im;i--)
	 ...
	 xmax=xp+size-1;
	 for(txl=0; txl<MXR_MAX;txl++) {if(text[txl]==EOS) break;}
	 k=xmax+4; if(wpis!=2) k=xmax+dltn;
	 if(k>X_maxBlank)
		{if(y<=Y_maxBlank && y<MY_MAX) {if(pznak>0) monit[pznak]=' '; return 0;}
		 x0+=(X_maxBlank-k);
		 if(x0<X_L0 && ms>=-x0) {monit[ms+im-x0]=EOS; x0=X_L0;}
		 if(x0<X_L0 && ms<-x0)  {monit[im]=EOS; x0=X_L0;}
		 xmax=x0+ms+size-1;
		}
	 if(string==NULL)
		{term_type(y,x0,&monit[im],0,attrtx); term_type(y,m_wherex()," NULL ",0,ATTR_A);
		 return m_wherex();
		}
	 ms=strlen(monit);
	 format=(char *)Malloc((xmax-x0+14)*sizeof(char));
	 if(format==NULL) {if(pznak>0) monit[pznak]=' '; return -1;}
	 lwmall++;
	 form=&format[8]; format[7]=EOS;
	 bufor=&form[ms+3*sizeof(char)]; bufor[0]=EOS;
	 sprintf(format," -%ds",size); format[0]='%';
	 textn[0]=EOS;
	 if(monit[0]!='#' && monit[1]!='#' || wpis==2)
		przepisz(bufor,string,dlstr+1); /* bylo sprintf(bufor,format,string);*/
	 Okno(y,x0,y,xmax,attr); x_konc=xmax+txl;
	 if(wpis!=1)
		{term_type(y,x0,&monit[im],0,attrtx);
		 ...
		 if(wpis==3)
			{if(help==NULL || l_opcji<=0 || y>=MY_MAX) {wpis=1; goto D;}
			 if(skok==0)
				{ret=okno_menu((char **)help[0], l_opcji, 0, attrtx, attr, y+1, xmax-size, xmax+1,
									 NULL, 0);
				}
			 else
				{shint w_min, w_max, size_helpu=0, sizh, k, xmax_h;
				 char *buf=NULL, index_helpu=0;
				 w_min=0; w_max=l_opcji-1;
				 k=term_save_image(y+1,xmax-size,taknie,0);
				 sizh=MX_MAX-xmax+size+1;  xmax_h=MX_MAX;
				 buf=(char *)Malloc(sizh*k+1); w_min=0;
				 if(buf!=NULL)
					{term_save_image(y+1,xmax-size,buf,sizh); lwmall++;
					 for(k=0;k<w_max;k++)
						{objasn_grupy(y+1,xmax-size,k,k,k,attr,attrtx,skok,dltn,NULL,help);
						 xmax_h=m_wherex(); if(w_min<xmax_h) w_min=xmax_h;
						}
					 xmax_h=w_min;
					 term_restore_image(y+1,xmax-size,buf,sizh); Free(buf); lwmall--;
					}
			 ...
			 if(ret>=0)
				{shint j;
				 ...
				 k=0;
				 if(skok==0) {tabl_opcji=(char **)help[0]; opcja=tabl_opcji[ret];}
				 else opcja=help[1]+ret*skok;
				 for(i=0,j=strlen(string);j<size;i++,j++)
					{string[j]=opcja[i]; k=x_konc;
					 if(opcja[i]==EOS) {string[j]=EOS; break;}
					}
				 string[size]=EOS;
				 x_konc=k;
				}
		...
		{for(i=0;i<txl;i++)
			{if(i+xmax+1<X_maxBlank) term_type(y,xmax+1+i,&text[i],1,attrtx);}
		 x_konc=m_wherex();
/*D:*/ sprintf(form,"%s s",monit); form[ms]='%';
WPIS:ins=INSERT; INSERT=1;
		 Okno(y,x0,y,xmax,attr); clr_Okno();
		 ret=bscanf(bufor,attrtx,form,textn);
		 INSERT=ins;
		 if(ret<0) {Free(format); lwmall--;
		 ...
		 if(ret>0)
			{for(i=0;i<=size;i++)
				{if(string[i]!=textn[i]) break;
				 if(string[i]==0) {i=size+1; break;}
				}
			 if(i>size) ret=0;
			}
		 if(ret==0) {strncpy(string,textn,size+1); Free(format); lwmall--;
								 if(pznak>0) monit[pznak]=' ';
								 return 0;
								}
		 ...
		 do{buffer=(char *)Malloc(dltn*k+1);
				...
				if(zn=='n') goto WPIS;
				if(zn=='t')
TAK:     {strncpy(string,textn,size+1);
					Term_Typef(y,xmax-size+1,string,size, attr); wpisano_dana=1;
				 }
			 }while(zn!=Esc && zn!='t');
		} /* else (wpis==1) */
D: Free(format); lwmall--;
	 if(pznak!=0) monit[pznak]=' ';
	 return x_konc;
 }
...
void def_haslo(char *hasl, char *kod)
 {char l;
  ...
  if(lhasel<LHASEL)
   {l=strlen(hasl); if(l>10) {hasl[10]=EOS; l=10;}
    przepisz(haslo[lhasel],hasl,l+1);
    l=strlen(kod); if(l>20) {kod[20]=EOS; l=20;}
...
char wpisz_haslo(char **kod_operat, signed char potrzebne_uprawn,
 ...
 {char poziom_upr, *tytul;
  ...
  if((potrzebne_uprawn%2)==1 && nr_oper!=0)
	 {komun_text(MY_MAX,X_L0," Zmiany mozna wprowadzic tylko Z TERMINALA GLOWNEGO !! Zostana ANULOWANE. <Ent> ",0,ATTR_A);
    GET_char();
    return 0;
   }
  ...
  if(Rp!=NULL && potrzebne_uprawn>=ZAP_MIN)   /* zamykamy raporty */
   {Okno(MY_MAX,X_L0,MY_MAX,MXR_MAX,attrtx); clr_Okno(); /*term_flush(); */
    ...
    term_flush();
    do{znak=GET_char();  znak=znak&0x00ff;}while(znak!=CR && znak!=Esc);
    if(znak==Esc) return 0;
   }
  while(potrzebne_uprawn>Uprawn_max)
   {do
     {Okno(MY_MAX,X_L0,MY_MAX,MXR_MAX,attrtx); clr_Okno();
      komun_text(MY_MAX,X_L0,tytul,0,TERM_WHITE|INVERSE|TERM_BLACK_BG);
      form=" WPISZ HASLO i zatwierdz je <Ent>: ?? ";
      l=strlen(form);
      i=MXR_MAX-10-l;
			Term_Typef(MY_MAX,i,form,l,attrtx); term_flush();
      Okno(MY_MAX,MXR_MAX-11,MY_MAX,MXR_MAX,attr_o); clr_Okno(); /*term_flush(); */
			term_cur(MY_MAX,MXR_MAX-10); komun_text(MY_MAX,MXR_MAX-10," ",0,attr_o); m_gotoxy(MXR_MAX-10,MY_MAX);
      buf[0]=EOS;
      ...
      for(i=0;i<lhasel;i++)
       {sukces=1;
        ...
        lh=strlen(haslo[i]); if(lh==0) continue;
        for(j=0;j<=lh;j++)
         {if(buf[j]!=haslo[i][j]) {sukces=0; break;}
          if(buf[j]==EOS) break;
         }
        if(sukces==1) break;
       }
...
"  ZA NISKIE UPRAWNIENIA: <Ent> -powtorka; <Esc> -wyjscie z anulow.zmian ?? ",0,ATTR_A);
    ...
    if(poziom_upr+ret>Uprawn_max)
     {Uprawn_max=poziom_upr+ret; Kod_operatora_max=*kod_operat;}
   }   /* koniec while()  */
  return 1;
 }
  ...
  while(potrzebne_uprawn>Uprawn_pocz)
   {do
     {Okno(MY_MAX,X_L0,MY_MAX,MXR_MAX,attrtx); clr_Okno();
      ...
      i=MXR_MAX-11-l;
			Term_Typef(MY_MAX,i,form,l,attrtx); term_flush();
      Okno(MY_MAX,MXR_MAX-10,MY_MAX,MXR_MAX,attr_o); clr_Okno(); term_flush();
      buf[0]=EOS;
			term_cur(MY_MAX,MXR_MAX-10); komun_text(MY_MAX,MXR_MAX-10," ",0,attr_o); m_gotoxy(MXR_MAX-10,MY_MAX);
      ret=czytaj_has(buf,0,10);
      if(ret<-1) return -1;
...
"ZA NISKIE UPRAWNIEN: <Ent> -powtorka; <Esc> -wyjscie z anulowaniem zmian: ?? ",0,ATTR_A);
    ...
    if(poziom_upr+ret>Uprawn_max)
     {Uprawn_max=poziom_upr+ret; Kod_operatora_max=NULL;
...
shint def_raport(signed char kod_raportu, shint anim_pid, shint ob_pocz,
             ...
             shint (*def_blankiet)(shint nr_ob, shint ob_pocz,
                                   shint ob_konc, shint xp, shint yp,
                                   shint anim_pid, char *D),
             char *(*dane_rap_bl)(shint ob_pocz, shint ob_konc, shint *rozmiar_ob),
             void (*wpis_rap_bl)(shint ob_pocz, shint ob_konc, char *D,
                      shint rozmiar_ob, char zapis[], char *Kod_op))
...
shint def_Report(signed char kod_raportu, shint anim_pid, shint ob_pocz,
 ...
 {shint i,j,l,k;
  ...
  if(typ_bazy!=BAZA_SPOJNA) {ob_konc=ob_konc-ob_pocz; ob_pocz=0;}
  if(ob_konc<ob_pocz)
  ...
  if(R!=NULL)                     /* przedefiniowanie raportu */
   {if(R->D!=NULL)
       ...
       {zapis_raportu(R,0,0);  /* !!! to moze dzialac zle, gdy UNDO wymaga kontroli */
        if(R->d_oryg!=NULL && (R->free_mem&0x01)==0x01)
         {l=R->ob_konc-R->ob_pocz+1;
          for(j=0;j<l;j++)
           {if(R->d_oryg[j]!=NULL)
             {Free(R->d_oryg[j]); R->d_oryg[j]=NULL; lwmall--;}
           }
          Free(R->d_oryg); lwmall--;
         }
        if((R->free_mem&0x01)==0x01) {Free(R->zapis); lwmall--; R->free_mem&=0xfe;}
			 }
	...
	R=RAP[i];
	if(R==NULL)
	 {shint zn=0, l=strlen(tytul);
		if(l>40) {zn=tytul[40]; tytul[40]=0;}
  ...
  R->ob_konc=ob_konc;
	...
	R->Kolor=Kolor;
	R->Ramka=Ramka;
	R->D=NULL; R->bufs=NULL;
	R->free_mem=0;
	R->zapis=NULL;
	R->uprawnienia=0; k=LRAP_BL;
...
/*      R->D=D-(r->ob_pocz-pocz)*rozmiar_ob; */
		...
		if(k==LRAP_BL)        /* nie jest to podraport */
		 {l=ob_konc-ob_pocz+1;
			...
			if(R->zapis==NULL)
			 {shint l=strlen(tytul), zn=0;
				if(l>30)  zn=tytul[30]; tytul[30]=0;
			 ...
			 {R->free_mem=0x01; lwmall++; for(i=0;i<l;i++) R->zapis[i]=0;
				...
				if(R->d_oryg==NULL)
				 {shint l=strlen(tytul), zn=0;
					...
					komunikat(MY_MAX,X_L0,ATTR_A,"%s. Brak pamieci na notatnik. Bedzie tylko bierny. <ENT> ",
															 tytul);
					if(zn!=0) tytul[30]=zn;
					Free(R->zapis); R->free_mem&=0xfe; R->zapis=NULL; lwmall--;
					R->uprawnienia=-1;
					GET_char();
				 }
				else {lwmall++; for(i=0;i<l;i++) R->d_oryg[i]=NULL;}
			 }
		 }
	 } /*  ================ koniec dla D!=NULL  */
 ...
 {if((R->free_mem&0x02)!=0x02) return R->D;
	if(R->typ_bazy==BAZA_WEKTOR)
	 {shint k;
		char **baza;
    baza=(char **)(R->D);
    for(k=0;k<=R->ob_konc;k++) {if(baza[k]!=NULL) {Free(baza[k]); lwmall--;}}
   }
  if(R->typ_bazy==BAZA_WEKTOR)
   {void *L;
    L=(void **)(R->D);
  ...
  Free(R->D); R->free_mem&=0xfd; lwmall--;
  return NULL;
 }

/* ----------------- 28.08.00 ----------------------------------- */
void zamknij_rap_kod(shint kod_raportu)
 {shint i=0, j, lrek, ret=0, Ret, poprz_dec=0, wymag_uprawn, zmiany;
  struct reports *R;
  for(i=0;i<LRAP_BL;i++)
   {R=RAP[i];
    if(R==NULL) continue;
...
/* komunikat(MY_MAX,X_L0,ATTR_A," Wszystk.zamkn: nr_blank=%d ",nr_blank); GET_char(); */
  ...
  if(blokada_zapisu==ZAPIS_WSZYSTKICH_RAZEM)
   {if(Klucz!=0) poprz_dec=UNDO;    /* tego nie powinno byc bo tu jest tylko jedno wywolanie */
     ...
     {poprz_dec=-1; wymag_uprawn=-1;  kontr_spojnosci=0;
      for(i=0;i<LRAP_BL;i++)
       {R=RAP[i];
        if(R==NULL) continue;
        zmiany=0;
        if(R->zapis!=NULL && (R->free_mem&0x01)==0x01)
         {lrek=R->ob_konc-R->ob_pocz+1;
          for(j=0;j<lrek;j++) {if(R->zapis[j]>=ZAP_MIN) {zmiany=1; break;}}
     ...
     {if(R->zapis!=NULL && (R->free_mem&0x01)==0x01)
       {if(poprz_dec==0) wymag_uprawn=R->uprawnienia;
        ...
        if(blokada_zapisu==ZAPIS_WSZYSTKICH_RAZEM && poprz_dec==-1)
         {if(ret<0) poprz_dec=UNDO; else poprz_dec=ZAP_OBOWIAZK;}
       }
     }
   }
  zamknij_grafiki(-1);
...
/* komunikat(MY_MAX,X_L0,ATTR_A," Wszystkie rap. zamkn"); GET_char(); */
  lwmallp=lwmall;
  NR_OPER=-1;   Uprawn_max=0; Kod_operatora_max=NULL;
	ZAPIS=0; BEZ_HASLA=0; EDIT=0; SLEDZIC=0; blokada_zapisu=ZAPIS_DOWOLNY;
  bez_potwierdzania=0;  Klucz=-1;
  blokada_potwierdzania=0;
#ifndef _DOS_
  if(mouse_free>0)
   {if(kproxy>0) qnx_proxy_detach(kproxy); kproxy=-1;
    if(Mouse_ctrl!=NULL)
     {mouse_close(Mouse_ctrl); Mouse_ctrl=NULL;
      if(Mproxy>0) qnx_proxy_detach(Mproxy); Mproxy=-1;
     }
	}
#endif
  mouse_free=0;
 ...
 {shint l;
  if(R->zapis==NULL)   /* dane nie sa z zewnatrz */
   {l=R->ob_konc-R->ob_pocz+1;
    R->zapis=Malloc(l*sizeof(char));
    if(R->zapis==NULL)
     {shint l=strlen(R->tytul), zn=0;
      if(l>30)  zn=R->tytul[30]; R->tytul[30]=0;
			komunikat(MY_MAX,X_L0,ATTR_A,"%s. Brak pamieci na notatnik. Bedzie tylko bierny. <ENT> ",
                             R->tytul);
      if(zn!=0) R->tytul[30]=zn;
      R->uprawnienia=0; R->d_oryg=NULL;
      GET_char();
     }
    else
     {R->free_mem|=0x01;  lwmall++;
      ...
      if(R->d_oryg==NULL)
       {shint l=strlen(R->tytul), zn=0;
        ...
        if((R->free_mem&0x01)==1 && R->zapis!=NULL)
         {Free(R->zapis); lwmall--; R->free_mem&=0xfe;}
        R->uprawnienia=-1;
        GET_char();
       }
      else {lwmall++; for(i=0;i<l;i++) R->d_oryg[i]=NULL;}
   ...
   {R->D=(R->dane_rap_bl)(R->ob_pocz, R->ob_konc, rozmiar_ob);
    if(R->D==NULL)
     {shint l=strlen(R->tytul), zn=0;
      ...
      if((R->free_mem&0x01)==1 && R->zapis!=NULL)
       {Free(R->zapis); lwmall--; R->free_mem&=0xfe;
        if(R->d_oryg!=NULL) {Free(R->d_oryg); lwmall--;}
       }
      Free(R); RAP[i]=NULL; lwmall--;
      GET_char(); return NULL;
     }
    lwmall++;
    R->free_mem|=0x02;
   }
  if(R->rozmiar_ob>0 && R->rozmiar_ob != *rozmiar_ob)
    ...
    if(R->zapis!=NULL && (R->free_mem&0x01)==0x01)
     {Free(R->zapis); R->zapis=NULL; lwmall--; R->free_mem&=0xfe;
      if(R->d_oryg!=NULL) {Free(R->d_oryg); lwmall--;}
     }
...
char *dane_raportowanego_rekordu(shint (*def_blank)(shint nr_ob, shint ob_pocz,
 ...
 {struct reports *R;
	 ...
	 if(BL[nr_blank]==NULL)
   {
P:  komunikat(MY_MAX,X_L0,ATTR_A," Otworz blankiet dla rekordu %d. <ENT> ",*nr_rekordu);
    GET_char(); *nr_rekordu=-1;
    return NULL;
   }
 ...
 {shint i, nr_bl, nr_rap;
  ...
  for(i=0;i<LRAP_BL;i++)
   {if(RAP[i]==NULL) continue;
    ...
    R->ob_konc=rek_konc;  R->ob_pocz=rek_pocz;
   }
 }

#define TXM 15
static int total_size=0;
/*struct lista {char *adr_nast; int size; shint rekord;}; w wewybl.h */
void *calloc_rep_mem(long int size, shint kod_raportu, shint rekord, char *txt)
 {long int i, ds;
	char *buf, *adr_buf, **poprz_adr, znak=0, k;
	struct reports *R=NULL;
	struct lista *l;
	if(strlen(txt)>TXM) {znak=txt[TXM]; txt[TXM]=0;}
	for(i=0;i<LRAP_BL;i++)
	 {if(RAP[i]==NULL) continue;
		if(RAP[i]->kod_raportu!=kod_raportu) continue;
		R=RAP[i]; break;
...
//			if(RAP[k]!=NULL) komunikat(MY_MAX-k,m_wherex(),TERM_WHITE|TERM_BLUE_BG," %s",RAP[k]->tytul);
		...
		if(znak!=0) txt[TXM]=znak; return NULL;
	 }
...
*/
  ...
  l->adr_nast=buf;
  l=(struct lista *)buf;
  l->adr_nast=NULL; l->size=size; l->rekord=rekord; total_size+=(size+ds);
...
struct lista *szukaj_adr_alloc(shint kod_raportu, char *txt,
  ...
  *liczba=0;
  ...
  l=(struct lista *)&R->bufs; ds=sizeof(struct lista);
  while(l->adr_nast!=NULL)
   {if((l->adr_nast+ds)==adr) break;
    l=(struct lista *)(l->adr_nast); (*liczba)++;
   }
...
void *free_all_rep_mem(struct reports *R)
 {shint i, liczba, ds;
  ...
  struct lista *l, *lp;
  lp=(struct lista *)&R->bufs; ds=sizeof(struct lista);
 ...
 {shint i, liczba;
  ...
  if(lp->adr_nast==NULL)
	 {komunikat(MY_MAX,X_L0,ATTR_A," Brak wskazanego adresu na liscie: %s(%d) dla rap.%d (tot.siz=%d) <Ent> " ,
                txt, size, kod_raportu, total_size);
		GET_char(); if(znak!=0) txt[TXM]=znak;
    return adres;
   }
  l=(struct lista *)lp->adr_nast; i=l->size; dalej=l->adr_nast;
  bufn=realloc(lp->adr_nast,size+sizeof(struct lista));
  if(bufn==NULL)
   {lp->adr_nast=dalej; total_size-=i; lwmall--;
...
void rekord_danych_do_naglowka(shint rekord_nr) {BL[nr_blank]->rek_danych_blank=rekord_nr;}

/* ------------------------------------------------------------------- */
shint Zapisz_raport(shint kod_raportu)
 {struct reports *R;
  shint i, poprz_dec=-1, wymag_uprawn=0;
   for(i=0;i<LRAP_BL;i++)
    {if(RAP[i]==NULL) continue;
     if(RAP[i]->kod_raportu!=kod_raportu) continue;
     R=RAP[i]; break;
 ...
 {signed char nr_raportu, i;
  ...
  if(poprz_dec<0)
   {if(BEZ_HASLA==0 && Rp!=NULL)
     {kod_operatora=text;
      ...
      if(zmiany==0) poprz_dec=UNDO; else poprz_dec=ZAP_OBOWIAZK;
     }
  ...
  if(R==NULL)    /* aktualny raport  */
     ...
     {if(poprz_dec==UNDO) return -1;
			 {komunikat(MY_MAX,0,ATTR_A," Brak blankietu. Zastosuj fkcje: Zapisz_raport(kod_raportu,%d,%d); <Ent> ",poprz_dec,wymag_uprawn);
        GET_char(); return 0;
       }
     }
		nr_raportu=BL[nr_blank]->nr_raportu;
    if(nr_raportu<0)
		 {komunikat(MY_MAX,X_L0,ATTR_A," Bledny numer raportu %d w blank.%d. <Ent> ",
															 nr_raportu,nr_blank);
      GET_char(); if(poprz_dec==UNDO) return -1; return 0;
     }
    R=RAP[nr_raportu];
    if(R==NULL)
  ...
  lrek=R->ob_konc-R->ob_pocz+1;
  zmiany=0; ret=0; kod_operatora=NULL;
  for(i=0;i<lrek;i++)
   {if(R->zapis[i]>=ZAP_MIN) {zmiany=1; break;}}
...
/* ======================= Kwestia hasla ============================= */
    ...
    if(poprz_dec==UNDO) {zmiany=0; zap_obowiazk=127; ret=-1;}
    if(zmiany==0)           /* ZAP_OBOWIAZK wynika z powiazan miedzy raportami */
     {for(i=0;i<lrek;i++)   /* == ta petla gwarantuje spojnosc danych w raporcie ==*/
       {if(R->zapis[i]>=ZAP_MIN && R->d_oryg[i]!=NULL)
         {if(R->zapis[i]>=zap_obowiazk) {zmiany=1; continue;}
          else
           {przepisz(R->D+i*R->rozmiar_ob,R->d_oryg[i],R->rozmiar_ob);
            R->zapis[i]=0; ret=-1;    /* jest UNDO */
     ...
     {(R->wpis_danych_bl)(R->ob_pocz, R->ob_konc, R->D, R->rozmiar_ob,
      ...
      for(i=0;i<lrek;i++)
       {R->zapis[i]=0;
        if(R->d_oryg!=NULL)
         {if(R->d_oryg[i]!=NULL)
           {Free(R->d_oryg[i]); R->d_oryg[i]=NULL; lwmall--;}
         }
       }
     }
   }
...
void reset_blankiet(shint nr_rekordu)
 {shint i, l;
  ...
  for(i=0;i<liczba_danych;i++,ok++)
   {if(ok->typ=='k' && ok->format!=NULL) {Free(ok->format); lwmall--;}
    ok->format=NULL; ok->wpis=0; for(l=0;l<LHELP;l++)  ok->help[l]=NULL;
   }
	R=RAP[BL[nr_blank]->nr_raportu];
  nr_danej=0;
...
shint okno_menu_rekordow(char *Menu[], shint w_max, shint opcja0,
 ...
 {struct reports Rd, *R;
	...
	shint size,/* wydruk nr'u rekordu gdy >0 */dim=0, xk, dl_tyt, tx_size, i;
	parinfo=&hformat+1;
	R=&Rd; R->D=(char *)Menu; R->typ_bazy=BAZA_WEKTOR;
	R->tytul=tytul; R->kod_raportu=-1; R->ob_pocz=opcja0; R->ob_konc=w_max;
	help[0]=hformat;
	dane_helpu(hformat, help, parinfo);
	tx_size=-1;
	if(tytul!=NULL)
	 {shint dl_tyt;
		dl_tyt=strlen(tytul); tx_size=Xp+dl_tyt+2;
	 }
	for(i=0;i<w_max;i++)
	 {xk=objasn_grupy(Yp, Xp, 0, w_max, 0, attr, attr, 1, 0, R, help);
		xk=m_wherex(); if(tx_size<xk) tx_size=xk;
	 }
	xk=tx_size+2;
	if(xk>Xm) Xm=xk;
...
/* -------------------------------------------------------------------- */
shint dana_znak(shint y, shint x, char *monit, char *opcje, char *objasn[],
...
shint tabl_znak(shint y, shint x, char *monit, shint ind_min, shint ind_max,
...
shint dana_double(shint y, shint x, char *monit, double *w_min, double *w_max,
...
shint tabl_grup_posredn(shint y, shint x, char *monit, shint ind_min,
 ...
 {char **parinfo;
  parinfo=&hformat+1;
  (Ok+nr_danej)->index_helpu=(shint *)index_helpu;
...
shint grupa_rap_posredn_danych_dec(shint y, shint x, char *monit, shint *w_min,
           shint *w_max, shint *wart, char *index_helpu,
           shint size_helpu, shint size, shint ochrona, shint skok, 
           signed char raport, shint kod_dec, char *hformat,...)
 {char **parinfo;
  parinfo=&hformat+1;
  (Ok+nr_danej)->index_helpu=(shint *)index_helpu;
  (Ok+nr_danej)->size_helpu=size_helpu;
...
shint grupa_danych(shint y, shint x, char *monit, shint *w_min,
									 shint *w_max, shint *wart, shint size, shint ochrona,
									 shint skok, signed char raport, char *hformat,...)
 {char **parinfo;
	parinfo=&hformat+1;
...
shint dana_rekord_posredn_str(shint y, shint x, char *monit, shint *w_min,
 ...
 {char **parinfo;
	...
	if(i==LRAP_BL) return nr_danej;
	(Ok+nr_danej)->Rep=R; skok=R->rozmiar_ob;
	if(R->typ_bazy==BAZA_SPOJNA) (Ok+nr_danej)->Rep=NULL;
	for(i=0;i<LRAP_BL;i++)
	 {R=RAP[i];
		if(R==NULL) return nr_danej;
...
----------------------------------- */
       ...
       {if(form[0]=='['&&form[1]=='%'&&(form[2]=='z'||form[2]=='Z'))
	      ...
	      if(*zn==EOS)
				 {komunikat(MY_MAX, X_L0,ATTR_A," Brak klamry ] lub liczby stringow w formacie %s grupy (k=%d). <Ent>", form,k);
    	    GET_char();
	        break;
	       }
	      if(typ!='s' && typ!='S')
...
shint tabl_int_menu(shint y, shint x, char *monit, shint ind_min,
 ...
 {shint ret;
  ...
  if(ret>0 && l_poz_menu>0 && menu !=NULL)
   {ok=Ok+ret-1; ok->help[0]=(char *)menu; ok->prec=l_poz_menu;}
  return ret;
 }
...
shint tabl_int_menu_dec(shint y, shint x, char *monit, shint ind_min,
 ...
 {shint ret;
  ...
  ret=tabl_int(y, x, monit, ind_min, ind_max, w_min, w_max,
               format_tabl, tablica, dim,  size, ochrona, raport);
  if(ret>0 && l_poz_menu>0 && menu !=NULL)
   {ok=Ok+ret-1; ok->help[0]=(char *)menu;
    ok->prec=l_poz_menu; ok->decyzja=kod_dec;
   }
  return ret;
 }
...
shint dana_int_menu(shint y, shint x, char *monit, shint *w_min, shint *w_max,
...
shint dana_int_menu_dec(shint y, shint x, char *monit, shint *w_min, shint *w_max,
                 shint *wart, shint size, shint ochrona, signed char raport,
...
shint dana_int(shint y, shint x, char *monit, shint *w_min, shint *w_max,
                 shint *wart, shint size, shint ochrona, signed char raport)
 {struct okno *ok;
   if(Ok==NULL)
		{komunikat(MY_MAX,X_L0,ATTR_A," Brak adresu dla danej %d blankietu. <Ent> ",
...
shint wpis_danej_text_menu(shint y, shint x, char *monit, char *string, shint size,
...
/* -------------------------------------------------------------------- */
shint blankiet(signed char *kod_raportu, shint *nr_ob, shint xp,
               shint yp, shint ym, shint xm, char czynny, signed char kolor,
               signed char ramka, char *poziom, char *zapis, shint *y_danej)
 {shint ret, Ret, rozmiar_ob, ob_pocz, ob_konc;
  signed char Kolor, Ramka;
  shint anim_pid;
	char *tytul, *D;
/* ----------------------------------------------------------------------- */
	Kolor=kolor; Ramka=ramka;
	for(;;)
		...
		*zapis=1;
		if(Ret!=FONT6)
		 {if(Ret==Esc || Ret==-CR || Ret==UNDO || Ret==TAB) *zapis=0;   /* juz zapisano */
			...
			if(nr_blank<0) break;
			if((*kod_raportu)<0) {Ret=-CR; break;}
			if(Ret==-1)
			 {/* Wyjscie z bledami */
				/* przepisz(d+(*nr_ob)*rozmiar_ob, Dr, rozmiar_ob); */
				*kod_raportu=zamknij_blankiet(nr_ob); break;
			 }
			{char format_inny_rek[32];
			 if(ob_pocz==ob_konc) sprintf(format_inny_rek,"º Chcesz WYJSC z RAPORTU ??? ");
			 else sprintf(format_inny_rek,"º <%c,%c,%c,%c,Home,End>-inny rek", 24,25,26,arrow);

#ifndef _DOS_
			 if(*zapis==1)
				{term_printf(MY_MAX,0,TERM_CYAN|INVERSE|TERM_FLUSH,
				 "º<Tab>-powrot do edycji º <%c,%c,%c,%c,Home,End>-inny rekº<F2>-ZAPIS º<Esc>-wyjscie º",
																	24,25,26,arrow);
				 term_printf(MY_MAX,24,ATTR_A,"%s%s",format_inny_rek,"º<F2>-ZAPISº");
				}
			 else
				{term_printf(MY_MAX,0,TERM_CYAN|INVERSE|TERM_FLUSH,
				 "º <Tab> - powrot do edycji raportu º <%c,%c,%c,%c,Home,End>-inny rekº<Esc>-wyjscie º",
																						 24,25,26,arrow);
				 term_printf(MY_MAX,X_L0+35,ATTR_A,"%s%s",format_inny_rek,"º");
        }
      }
      if(Ret==-CR)
        komun_text(MY_MAX,0,"ºPREZENTACJA DANYCH bez prawa zmian",0,TERM_CYAN|INVERSE|TERM_FLUSH);
#else
      if(*zapis==1)
       {term_printf(MY_MAX,0,TERM_CYAN|INVERSE|TERM_FLUSH,
        "º<Tab>-powrot do edycji º <%c,%c,%c,%c,Home,End>-inny rekº<F2>-ZAPISº<Esc>-wyjscieº",
																		24,25,26,arrow);
				term_printf(MY_MAX,X_L0+24,ATTR_A,"%s%s",format_inny_rek,"º<F2>-ZAPISº");
       }
      else
       {term_printf(MY_MAX,0,TERM_CYAN|INVERSE|TERM_FLUSH,
        "º <Tab> - powrot do edycji raportu º <%c,%c,%c,%c,Home,End>-inny rekº<Esc>-wyjscieº",
                                             24,25,26,arrow);
				term_printf(MY_MAX,X_L0+35,ATTR_A,"%s%s",format_inny_rek,"º");
       }
      }
...
shint Esc_Tab_wybor_toru(shint Ret, signed char *kod_raportu, shint *nr_ob,
                  shint ob_pocz, shint ob_konc, char *zapis, char *poziom,
                  shint *y_danej)
 {shint ret;
		*poziom=nr_blank;
		if(nr_blank>=0)
					{ret=BL[nr_blank]->nr_danej; *y_danej=(Ok+ret)->yo+Y_0;}
    if(Ret==FONT1)
...
/*        case CR:  goto F2;  */
        case BS:   if(ob_pocz==ob_konc) continue;
                   (*nr_ob)--; if((*nr_ob)<ob_pocz) (*nr_ob)=ob_konc;
                   ret=1; break;
        case SPEC:
			 ret=GETchar(); /*fprintf(mystderr," %d", ret); */
         if(ob_pocz==ob_konc && ret!=FONT2) continue;
         switch(ret)
          {case Home: (*nr_ob)=ob_pocz; ret=1; break;
           case End:  (*nr_ob)=ob_konc; ret=1; break;
           case PgUp: (*nr_ob)+=5;
           ...
           case PgDn: (*nr_ob)-=5;
                      ...
                      ret=1; break;
           case LEFT:
           case DOWN: (*nr_ob)--; if((*nr_ob)<ob_pocz) (*nr_ob)=ob_konc;
                      ret=1; break;

           case FFR:
           case UP :(*nr_ob)++; if((*nr_ob)>ob_konc) (*nr_ob)=ob_pocz;
                      ret=1; break;
           case FONT2:
             if(Ret==-CR) continue;
             if(*zapis==0) continue;
...
char jaka_decyzja(shint *kod, shint *nr_danej)
 {*kod=-1; *nr_danej=-1;
    ...
    *kod=(Ok+(*nr_danej))->decyzja;
    if(((Ok+(*nr_danej))->typ=='F'||(Ok+(*nr_danej))->typ=='f'||(Ok+(*nr_danej))->typ=='d'||(Ok+(*nr_danej))->typ=='w'||(Ok+(*nr_danej))->typ=='W'||(Ok+(*nr_danej))->typ=='o') && (Ok+(*nr_danej))->proxy==0)
		 {*kod=_GRAFIKA_; return (Ok+(*nr_danej))->typ;}
   }
...
shint rap_blank(char *tytul, signed char *kod_raportu, shint *nr_rekordu)
 {shint no, y, x, yp, xp, ret, wp, l_oblig=0, dana=0, znak, d0, y_shift;
	struct okno *ok;
	signed char nr_raportu;
	shint rozmiar_ob, l_danych, ypocz;
	char x_tyt, y_tyt, *d_oryg=NULL, *d_rek=NULL, *zmiany;
	static shint wpisano=0;
	...
	if(l_danych==0)
	 {komunikat(5,1,ATTR_A," Brak danych blankietu %d: <Ent>", nr_blank);
		GET_char(); return -1 /*zamknij_blankiet()*/;
	 }
/*  nr_danej=0; */
	ABORT=0; BLAD=0;
	...
	if(BL[nr_blank]->ramka==1)
	 {Okno(Y_0-1,X_0-1,Y_maxBlank+1,X_maxBlank+1,text_a|tlo);
		maluj_ramke(Y_0-1,X_0-1,Y_maxBlank+1,X_maxBlank+1,text_a|tlo); y_tyt=Y_0-1;
	 }
	...
	attr_o[0]=attr_ok; attr_o[1]=attr_o[0];
	attr_dat[0]=attr_d; attr_dat[1]=attr_dat[0]; attr_dat[2]=attr_dat[0];
	attr_o[2]=attr_wp;         /*attr_d|TERM_MAGENTA_BG; */
	attr_dat[2]=attr_o[2];
	attrtx[0]=text_a|tlo; attrtx[1]=text_a|tlo|INVERSE;
	attrtx[2]=attrtx[1];
/* ------------------------------------------------------------------ */
...
/*  wpisano=0; */
	ypocz=m_wherey();
	wp=0;  ok=Ok;  ret=0; l_oblig=0;    /* wyswietlenie calego blankietu */
	ret=X_0; /* -1;bylo do grudn.97 */
	m_gotoxy(X_0,Y_0); y_shift=0;
/*fprintf(mystderr,"\nOK=%d Ok->form=%s !!!",Ok,Ok->format); */
	for(no=0;no<l_danych;no++,ok++)
	 {if(ok->format==NULL) {l_danych=no; break;}
/*    Okno(Y_0,X_0,Y_maxBlank,X_maxBlank,text_a|tlo);  / * to popr. z grudn.97 */
		yp=m_wherey();
		xp=ret;                   /* +1; tak bylo do grudn.97  */
		x=ok->xo+X_0; y=ok->yo+Y_0; if(y<yp) y=yp;
		if(x<X_0) x=xp;
		else {if(x<xp) y=yp+1;}
		if(y<ok->yo+Y_0) y=ok->yo+Y_0;
		if(y>Y_maxBlank)
		 {if(BL[nr_blank]->scroll<0) {l_danych=no+1; y=Y_maxBlank;}
...
/*fprintf(mystderr,"\nSCROLL: y=%d Y_maxBlank=%d l_danych=%d/%d",y,Y_maxBlank,l_danych,liczba_danych); */
		...
		wp=0;
		attr=attr_o[0];
		if(ok->ochrona>Uprawn) {attr=attr_inf; wp=2;}
...
/* to odblokowac dla sledzenia */
		...
		setcursor(nocursor);
		do{ret=disp_dana(wp, y-y_shift, x, attr_d, attr, attrtx[0], ok, 0, 0);
			 if(Y_maxBlank<0) Y_maxBlank=-Y_maxBlank;
			 if(ret==-1) {wyjscie(Ok,liczba_danych); return -1;}
			 if(ret<=0)
				{if(y==yp)
					{y++; x=X_0;
					 if(y>=Y_maxBlank)
						{if(BL[nr_blank]->scroll<0) {l_danych=no; break;}
						 else y_shift=Y_maxBlank-y;
						}
					}
				 else {if(x>X_0) x=X_0;
		...
		ok->yo=y-Y_0; ok->xo=x-X_0;	ok->xmax=ret;
		if(ok->yplus==1)
		 {y++; if(y>Y_maxBlank) y=Y_maxBlank; ret=X_0; /*-1; do grudn.97*/
			term_cur(y,X_0);
		 }
/*fprintf(mystderr, " no=%d y=%d form=%s;",no,ok->yo,ok->format); */
	 }
	if(wpisano==UNDO)         /* Po anulowaniu zmian  */
	 {wyjscie(Ok,liczba_danych); wpisano=0; return Esc;}
	wpisano=0;
	nr_raportu=BL[nr_blank]->nr_raportu;
   ...
   {for(d0=dana+1;d0!=dana;d0++)
		 {if(d0>=l_danych) d0=0; if(d0==dana) break;
      ok=Ok+d0; if(ok->ochrona<=Uprawn || ok->blok_wpisu==1) break;
     }
    if(d0==dana)
...
*/
      ...
      return -CR;
		...
		dana=d0; BL[nr_blank]->nr_danej=d0;
   }
    ...
    *nr_rekordu=BL[nr_blank]->nr_rekordu;
    ...
    if(znak==FONT1)
       ...
       {if((Ok+dana)->typ=='g'||(Ok+dana)->typ=='G') /* raport dotyczy helpu */
         (*nr_rekordu)=*(shint *)((char *)(Ok+dana)->index_helpu
                      +(*(shint *)(Ok+dana)->wartosc)*(Ok+dana)->size_helpu);
        else znak=FONT6;
       }
      else *nr_rekordu=*(shint *)(Ok+dana)->wartosc;
     }
		if(znak==FONT6) return znak;
/* ============================================================== */
    if(wpisano<ZAP_MIN)
		 {wyjscie(Ok,liczba_danych);  return znak; /* bylo ret */}
/* ----------------- cos zostalo wpisane ---------------------------- */
#ifndef _DOS_
		term_type(MY_MAX,X_L0,
"º<Tab>-powrot do danychº<F2> -ZAPISº<q> -WYJSCIEº<Esc> -wyjscie Z ANULOW.ZMIAN º",0,attr_h);
#else
		term_type(MY_MAX,X_L0,
"º<Tab>-powrot do danychº<F2> -ZAPISº<q> -WYJSCIEº<Esc> -wyjscie Z ANULOW.ZMIANº",0,attr_h);
		term_type(MY_MAX,24,"º<F2> -ZAPISº<q> -WYJSCIEº",0,ATTR_A);
#endif
		do
		 {znak=GET_char();  znak=znak&0x00ff;
      if(znak==Esc)     /* porzucenie blankietu, do menu   */
       {if(wpisano>=ZAP_MIN)
         {term_type(MY_MAX,X_L0,
								 "º  Chcesz zakonczyc raport BEZ WPISU !!! <t/n> ?? º",0,ATTR_A);
					do{znak=GET_char();  znak=znak&0x00ff;}while(znak!='t' && znak!='n');
          if(znak=='n') {znak=TAB; continue;}
          if(znak=='t')
           {przepisz(d_rek,d_oryg,rozmiar_ob); *zmiany=0;
            if(wpisano>ZAP_MIN+1)   /* #### ???? ##### */
              return FONT6;  /* zmieniono decyzje - powrot do poprzedniej */
						term_cur(ypocz,X_0);
            wpisano=UNDO; return UNDO;  /* wyswietlenie poprawionego raportu i wyjscie */
           }
         }
        wyjscie(Ok,liczba_danych);
/* na zewnatrz mozna wejsc na inny rekord  */
      ...
      if(znak=='q')           /* wyjscie bez zapisu, ale bez anul.danych */
       {wyjscie(Ok,liczba_danych); return TAB; /*Esc; / * bylo TAB */}
      if(znak==SPEC || znak==EOS)
			 {znak=GETchar();  znak=znak&0x00ff;
        if(znak==SHIFT_TAB || znak==CTRL_TAB)
				 {przelacz_potwierdzanie(); continue;}
        if(znak==FONT2 && wpisano>=ZAP_MIN)
         {if(l_oblig==0)
           {wyjscie(Ok,liczba_danych);
            ret=zapis_raportu(NULL,0,0);
            if(ret<0) {wpisano=UNDO; return UNDO;}
						return Esc;
           }
          term_type(MY_MAX,X_L0,
"ºWpisz dane OBLIGATORYJNE (ze znakiem ?? ) -<Ent>; lub wyjdz bez zapisu -<q>º",0,ATTR_A);
          znak=GET_char();  znak=znak&0x00ff;
          if(znak==CR) znak=TAB;       /* powrot do edycji */
          if(znak=='q') {wyjscie(Ok,liczba_danych); return TAB;}
         }
       }
     ...
     {shint zn=0, l=strlen(tytul);
      if(l>30) {zn=tytul[30]; tytul[30]=0;}
			komunikat(MY_MAX,1,ATTR_A,"%s. Brak miejsca na dane. Bedzie bierny  !! <Ent> ",
													 tytul);
      if(zn!=0) tytul[30]=zn;
      GET_char();  *rozmiar=0; *zmiany=NULL;
      return NULL;
     }
    R->d_oryg[nr_rekordu-R->ob_pocz]=d_or;   lwmall++;
...
char *czy_jest_blankiet(signed char kod_raportu, shint *anim_pid, shint *nr_ob,
 ...
 {shint ret, ob_p, ob_k, nr_rap, kod_rap, nr_rek;
	signed char kolor, ramka, nrdan=-1;
	char *Dane;
	shint (*def_blankiet)(shint, shint, shint, shint, shint, shint, char *);
	 ...
	 {if(nr_blank<0) Czy_Blankiet_otwarty=0;
		 ...
		 {nr_rap=BL[nr_blank]->nr_raportu;
			...
			if(BL[nr_blank]->reset>0)
			 {nrdan=BL[nr_blank]->nr_danej; BL[nr_blank]->reset=0;
...
*/
			...
			if(kod_rap!=kod_raportu) {Czy_Blankiet_otwarty=0; nrdan=-1;}
			else
...
D:def_blankiet=czy_jest(kod_raportu, anim_pid, ob_pocz, ob_konc,
	...
	switch(Czy_Blankiet_otwarty)
	 {case 0:
		 ...
		 ret=otworz_blankiet(kolor,ramka, yp, xp, ym, xm, kod_raportu, *nr_ob,
												 czynny);
		 if(ret==-1) {term_type(MY_MAX,X_L0,
								" Nie mozna otworzyc blankietu !! <Ent> ",0,ATTR_A);
								zamknij_raporty();
								GET_char(); return NULL;
							 }
		 if(ret==0) {zamknij_raporty(); return NULL;}
		 if(ret==-2)
			{Czy_Blankiet_otwarty=2;
...
*/
		 break;
		case 2: nr_danej=BL[nr_blank]->nr_danej; return Dane;
		default:
	...
	BL[nr_blank]->y_scroll=-1;   BL[nr_blank]->scroll=-1;
	ret=def_blankiet(*nr_ob, ob_p, ob_k, xp, yp, kod_raportu, Dane);
	if(SCROLL>=0) {BL[nr_blank]->scroll=SCROLL; SCROLL=-1;}
...
*/
    ...
    GET_char();
    zamknij_raporty(); return NULL;
   }
  Czy_Blankiet_otwarty=2;
  if(nrdan>=0) {nr_danej=nrdan; BL[nr_blank]->nr_danej=nrdan;}
  else nr_danej=0;
...
/* ------------------------------------------------------------ */
void wysylka(shint dana, shint kod, float wartosc, shint proxy)
 {dana=dana; kod=kod; wartosc=wartosc; proxy=proxy;};
/* =================================================================== */
shint obsluga_wpisu(shint dana, struct okno *Ok, shint liczba, unsigned int attr_dat[],
                    unsigned int attr_o[], unsigned int attrtx[], unsigned int attr_h,
                    unsigned int attr_inf, shint *loblig, signed char *raport,
                    shint *wpisano)
 {shint znak, stara, nowa, ret, l_oblig, z_menu, y, x, yo, xo, ind, y_shift=0;
...
/*   *raport=-1; */
   ...
   nowa=dana;
   mod=1; ok=Ok+dana; znak=0; wp=0; help=0;
   do             /* petla po mod (po zmianie danej) do nastepnej zmiany */
    {
Q:   ok=Ok+dana;  /* ustawienie na pozycji dana */
     if(mod==0) {wp=0; if(ok->wpis==1 || ok->blok_wpisu==1) wp=2;}
/*if(ok->typ=='F') fprintf(mystderr,"\nwp=%d %s ",wp,ok->format); */
		 z_menu=0;
P:   ret=disp_dana(wp, ok->yo+Y_0, ok->xo+X_0, attr_dat[mod], attr_o[mod], attrtx[mod], ok, y_shift, mod);
		 if(wp<-2) wp=-wp;
		 ...
		 if(wp==1 || wp==-1 || (wp==4 && wpisano_dana>0))
			{if(ret>0 || z_menu>0 || wpisano_dana>0)
				 ...
				 *wpisano=1;
				 if(ok->ochrona>0)
					{*wpisano=2; if((*wpisano)<ZAP_MIN) (*wpisano)=ZAP_MIN;}
				 if(ok->decyzja>=0)
...
"º  Wpis danej: <Esc> - wyjscieº  <Ent> - akceptacja º <%c, %c> - inkrementacja   º",
                           24,25);
#else
        {term_printf(MY_MAX,X_L0, attr_h,
"º  Wpis danej: <Esc> - wyjscieº  <Ent> - akceptacja º <%c, %c> - inkrementacja  º",
...
*/
          ...
          if(ok->typ=='W'||ok->typ=='F')
           {help=4; Tabl=1; if(ok->proxy==0) help=14;}
          ...
          if((ok->typ=='I'||ok->typ=='G') && ok->kod_anim>=0) help+=1;
					...
					switch(help)
					 {case 7:           /* help= 1+2+4- raport+menu+tablica */
							 term_printf(MY_MAX,X_L0,attr_h,
									"º<%c,%c,%c,%c>-danaº<F1>-raportº<F3>-menuº<F4>-tabl",
													 24,25,26,arrow);
							 if(ok->blok_wpisu==0) term_type(MY_MAX,m_wherex(),    "º<ENT>-edycja º<Esc,Tab>-wyjscie º",0,attr_h);
							 else                  term_type(MY_MAX,m_wherex(),    "ica  º <Esc,Tab>-wyjscie z rap.  º",0,attr_h);
							 break;
						case 6:           /* help= 2+4- menu+tablica */
							 term_printf(MY_MAX,X_L0,attr_h,
									"º<%c,%c,%c,%c>-wybor danejº<F3>-menu º<F4>-tablica",
                           24,25,26,arrow);
               if(ok->blok_wpisu==0) term_type(MY_MAX,m_wherex(),    "º<ENT>-edycja º<Esc,Tab>-wyjscie º",0,attr_h);
               else                  term_type(MY_MAX,m_wherex(),    "  º <Esc,Tab>-wyjscie z raportu  º",0,attr_h);

               break;
            case 5:           /* help= 1+4- raport+tablica */
               term_printf(MY_MAX,X_L0,attr_h,
                  "º<%c,%c,%c,%c>-wybor danejº<F1>-raportº<F4>-tablica",
                           24,25,26,arrow);
               if(ok->blok_wpisu==0) term_type(MY_MAX,m_wherex(),    "º<ENT>-edycja º<Esc,Tab>-wyjscie º",0,attr_h);
               else                  term_type(MY_MAX,m_wherex(),    "  º <Esc,Tab>-wyjscie z raportu  º",0,attr_h);
            ...
            case 4:           /* help= 4- tablica */
               term_printf(MY_MAX,X_L0,attr_h,
                  "º<%c,%c,%c,%c,Home,End>-wybor danej º<F4>- tablica",
													 24,25,26,arrow);
               if(ok->blok_wpisu==0) term_type(MY_MAX,m_wherex(),    "º<ENT>-edycja º<Esc,Tab>-wyjscie º",0,attr_h);
               else                  term_type(MY_MAX,m_wherex(),    " º <Esc,Tab> - wyjscie z raportu º",0,attr_h);
               break;
            case 14:          /* help= 14- tablica i grafika */
               term_printf(MY_MAX,X_L0,attr_h,
                  "º<%c,%c,%c,%c,Home,End>-wyborº<F1>-wykr.º<F4>-tabl",
                           24,25,26,arrow);
               if(ok->blok_wpisu==0) term_type(MY_MAX,m_wherex(),    "º<ENT>-edycja º<Esc,Tab>-wyjscie º",0,attr_h);
							 else                  term_type(MY_MAX,m_wherex(),    " º <Esc,Tab> - wyjscie z raportu º",0,attr_h);
               break;
            case 3:
               term_printf(MY_MAX,X_L0,attr_h,
									"º<%c,%c,%c,%c>-wybor danejº<F1>-raport º<F3>-menu ",
                           24,25,26,arrow);
               if(ok->blok_wpisu==0) term_type(MY_MAX,m_wherex(),   "º<ENT>-edycja º<Esc,Tab>-wyjscie º",0,attr_h);
               else                  term_type(MY_MAX,m_wherex(),   "º <Esc,Tab> - wyjscie z raportu  º",0,attr_h);
               break;
            case 2:
               term_printf(MY_MAX,X_L0,attr_h,
                  "º<%c,%c,%c,%c,Home,End>-wybor danej º<F1,F3>-menu ",
                           24,25,26,arrow);
							 if(ok->blok_wpisu==0) term_type(MY_MAX,m_wherex(),   "º<ENT>-edycja º<Esc,Tab>-wyjscie º",0,attr_h);
               else                  term_type(MY_MAX,m_wherex(),   " º <Esc,Tab> - wyjscie z raportu º",0,attr_h);
               break;
            case 1:
							 term_printf(MY_MAX,X_L0,attr_h,
                  "º<%c,%c,%c,%c,Home,End>-wybor danej º<F1>-raport ",
                           24,25,26,arrow);
               if(ok->blok_wpisu==0) term_type(MY_MAX,m_wherex(),   "º<ENT>-edycja º<Esc,Tab>-wyjscie º",0,attr_h);
               else                  term_type(MY_MAX,m_wherex(),   " º <Esc,Tab> - wyjscie z raportu º",0,attr_h);
               break;
            case 0:
               term_printf(MY_MAX,X_L0,attr_h, /*TERM_CYAN|INVERSE|TERM_FLUSH,*/
                  "º<%c,%c,%c,%c, Home, End> - wybor danej ",
													 24,25,26,arrow);
               if(ok->blok_wpisu==0) term_type(MY_MAX,m_wherex(),   "º<ENT> - edycja danej º<Esc,Tab> - wyjscie º",0,attr_h);
               else                  term_type(MY_MAX,m_wherex(),   "do prezent.º <Esc,Tab> - wyjscie z raportu º",0,attr_h);
               break;
						case 16:
               term_printf(MY_MAX,X_L0,attr_h, /*TERM_CYAN|INVERSE|TERM_FLUSH,*/
                  "º<%c,%c,%c,%c,Home,End>-wyborº<F1>-wykr.",
                           24,25,26,arrow);
               if(ok->blok_wpisu==0) term_type(MY_MAX,m_wherex(),   "º<ENT> - edycja danej º<Esc,Tab> - wyjscie º",0,attr_h);
               else                  term_type(MY_MAX,m_wherex(),   "do prezent.º <Esc,Tab> - wyjscie z raportu º",0,attr_h);
            default: break;
					...
					switch(help)
					 {case 7:           /* help= 1+2+4- raport+menu+tablica */
							 term_printf(MY_MAX,X_L0,attr_h,
									"º<%c,%c,%c,%c>-danaº<F1>-raportº<F3>-menuº<F4>-tabl",
													 24,25,26,arrow);
							 if(ok->blok_wpisu==0) term_type(MY_MAX,m_wherex(),    "º<ENT>-edycjaº<Esc,Tab>-wyjscieº",0,attr_h);
							 else                  term_type(MY_MAX,m_wherex(),    "ica  º <Esc,Tab>-wyjscie z rap.º",0,attr_h);
							 break;
						case 6:           /* help= 2+4- menu+tablica */
							 term_printf(MY_MAX,X_L0,attr_h,
									"º<%c,%c,%c,%c>-wybor danejº<F3>-menu º<F4>-tablica",
													 24,25,26,arrow);
							 if(ok->blok_wpisu==0) term_type(MY_MAX,m_wherex(),    "º<ENT>-edycjaº<Esc,Tab>-wyjscieº",0,attr_h);
							 else                  term_type(MY_MAX,m_wherex(),    "  º <Esc,Tab>-wyjscie z raportuº",0,attr_h);
						...
						case 14:          /* help= 14- tablica i grafika */
							 term_printf(MY_MAX,X_L0,attr_h,
									"º<%c,%c,%c,%c,Home,End>-wyborº<F1>-wykr.º<F4>-tabl",
													 24,25,26,arrow);
							 if(ok->blok_wpisu==0) term_type(MY_MAX,m_wherex(),    "º<ENT>-edycja º<Esc,Tab>-wyjscieº",0,attr_h);
							 else                  term_type(MY_MAX,m_wherex(),    " º <Esc,Tab> - wyjscie z raportuº",0,attr_h);
							 break;
						case 3:
							 term_printf(MY_MAX,X_L0,attr_h,
						...
						case 0:
							 term_printf(MY_MAX,X_L0,attr_h, /*TERM_CYAN|INVERSE|TERM_FLUSH,*/
									"º<%c,%c,%c,%c, Home, End> - wybor danej ",
													 24,25,26,arrow);
							 if(ok->blok_wpisu==0) term_type(MY_MAX,m_wherex(),   "º<ENT> - edycja danej º<Esc,Tab> - wyjscieº",0,attr_h);
							 else                  term_type(MY_MAX,m_wherex(),   "do prezent.º <Esc,Tab> - wyjscie z raportuº",0,attr_h);
							 break;
            case 16:
               term_printf(MY_MAX,X_L0,attr_h, /*TERM_CYAN|INVERSE|TERM_FLUSH,*/
                  "º<%c,%c,%c,%c,Home,End>-wyborº<F1>-wykr.",
...
/* klawisze: <UP, DOWN, RIGHT, LEFT, HOME, END, CR, Esc, TAB>  */
           ...
           {znak=GET_char();  znak=znak&0x00ff;
            if(znak==Esc)     /* porzucenie blankietu, do menu   */
             {mod=0; wp=0; if(ok->wpis==1 || ok->blok_wpisu==1) wp=2;
							goto Q;
             }
...
/*fprintf(mystderr,"\nTabl=%d ok->help[2]=%d ",Tabl,ok->help[2]); */
                if(Tabl==1 && ok->help[2]!=NULL) {wp=-2; help=0; Tabl=0;}
                else continue;
							 }
              else {mod=2; wp=1; if(Tabl==1) wp=-1; help=0; Tabl=0;}
#ifndef _DOS_
              term_printf(MY_MAX,X_L0,attr_h, /*TERM_CYAN|INVERSE|TERM_FLUSH,*/
"º  Wpis danej: <Esc> - wyjscieº  <Ent> - akceptacja º <%c, %c> - inkrementacja   º",
                           24,25);
...
"º  Wpis danej: <Esc> - wyjscieº  <Ent> - akceptacja º <%c, %c> - inkrementacja  º",
            ...
            if(znak==EOS || znak==SPEC)
             {znak=GETchar();  znak=znak&0x00ff;
              if(znak==SHIFT_TAB || znak==CTRL_TAB)
               {przelacz_potwierdzanie(); goto DALEJ;}
              if(znak==PgUp) {nowa-=4; znak=UP; goto W_LEWO;}
              if(znak==PgDn) {nowa+=4; znak=DOWN; goto W_PRAWO;}
              if(znak==Home) {nowa=-1; znak=DOWN; goto W_PRAWO;}
              if(znak==End) {nowa=liczba; znak=UP; goto W_LEWO;}
              if(znak==DOWN)
               {ok=Ok+nowa; yo=ok->yo; xo=ok->xo; /* przechodz.do nast.linii */
                do{nowa++; if(nowa>=liczba) nowa-=liczba; ok=Ok+nowa;
/*fprintf(mystderr," nowa=%d stara=%d",nowa,stara); */
                  }while((ok->ochrona>Uprawn && ok->blok_wpisu==0 || ok->yo==yo)&& nowa!=stara);
                if(stara==nowa) break;
                yo=ok->yo; x=abs(ok->xo-xo);
                for(ind=nowa+1;ind<liczba;ind++)
                 {ok=Ok+ind; if(ok->yo!=yo) break;
                  if(ok->ochrona>Uprawn && ok->blok_wpisu==0) continue;
                  if(abs(ok->xo-xo)<x) {x=abs(ok->xo-xo); nowa=ind;}
                  else break;
                  }
                goto DALEJ;
               }
              if(znak==FFR)
W_PRAWO:       {do{nowa++; if(nowa>=liczba) nowa-=liczba; ok=Ok+nowa;
/*fprintf(mystderr," nowa=%d stara=%d",nowa,stara); */
              ...
              if(znak==UP)
               {ok=Ok+nowa; yo=ok->yo; xo=ok->xo; /* przechodz.do nast.linii */
                do{nowa--; if(nowa<0) nowa=liczba+nowa; ok=Ok+nowa;
/*fprintf(mystderr," nowa=%d stara=%d",nowa,stara); */
                  }while((ok->ochrona>Uprawn && ok->blok_wpisu==0 || ok->yo==yo) && nowa!=stara);
                if(stara==nowa) break;
                yo=ok->yo; x=abs(ok->xo-xo);
              ...
              if(znak==BBS)
               {
W_LEWO:         do{nowa--; if(nowa<0) nowa=liczba+nowa; ok=Ok+nowa;
/*fprintf(mystderr," nowa=%d stara=%d",nowa,stara); */
                  }while(ok->ochrona>Uprawn && ok->blok_wpisu==0 && nowa!=stara);
DALEJ:          if(nowa==stara) break; /* bylo continue; */
								mod=0; dana=stara; wp=0;
                if(ok->wpis==1 || ok->blok_wpisu==1) wp=2;
                break;
               }
              if(help>0)
               {if((help&0x01)==1 && znak==FONT1)
                 {*raport=ok->kod_anim; help=0;
                  BL[nr_blank]->nr_danej=dana;
                  return FONT1;
                 }
                if(help==14 || help==16)  /* FONT1 dla grafiki gdy sa dane 'f' lub 'F' lub 'd' */
                 {if((ok->typ=='f' || ok->typ=='F' || ok->typ=='d') && ok->proxy==0 && znak==FONT1)
                   {help=0; BL[nr_blank]->nr_danej=dana;
										return FONT1;
                   }
                 }
                if((help&0x02)==2&&(znak==FONT3||znak==FONT1))
                 {mod=2; wp=3; if(ok->blok_wpisu==1) wp=-3;
#ifndef _DOS_
                  term_printf(MY_MAX,X_L0,attr_h,
"º<%c,%c,%c,%c,Home,End>-wybor opcji  º <Ent> - akceptacja º <Esc> - wyjscie z menu  º",
                           24,25,26,arrow);
#else
                  term_printf(MY_MAX,X_L0,attr_h,
"º<%c,%c,%c,%c,Home,End>-wybor opcji  º <Ent> - akceptacja º <Esc> - wyjscie z menuº",
                           24,25,26,arrow);
#endif
                ...
                if((help&0x04)==4&&(znak==FONT4))
                 {mod=2; wp=4; /* if(ok->blok_wpisu==1) wp=-4; ### */
									if(ok->blok_wpisu==1) wp=5;   /* wpis w tabl.dodane 27.01.98 */
#ifndef _DOS_
                   term_printf(MY_MAX,X_L0,attr_h,
"º<%c,%c,%c,%c,Home,End> - wybierz do edycji º <Ent> - akceptacja º <Esc> - wyjscie º",
                           24,25,26,arrow);
#else
                  term_printf(MY_MAX,X_L0,attr_h,
"º<%c,%c,%c,%c,Home,End> - wybierz do edycji º <Ent> - akceptacja º <Esc> - wyjscieº",
                           24,25,26,arrow);
#endif
                  if(ok->blok_wpisu==1)
                    term_type(MY_MAX,X_L0+1," Przeglad danych bez prawa ich edycji  ",0,attr_o[2]);
									else term_printf(MY_MAX,X_L0+1,attr_o[2],"<%c,%c,%c,%c,Home,End> - wybierz do edycji ",
                              24,25,26,arrow);
                 }
                znak=CR; help=0;
                break;
       ...
       case 2:      /* po wczytaniu danej */
          ...
          if(ret>=0)
					 {ok->wpis=1; wp=2;
            if(ret>0)
             {if(ok->proxy>0) wysylka(dana, ok->kod_anim, *(ok->wartosc),
                                      ok->proxy);
...
/*fprintf(mystderr,"\n%s ",okd->format); */
  for(no=dana+1;no!=dana;no++)
   {if(no==liczba) no=0; if(no==dana) break;
    ok=Ok+no;
...
void animuj_blank(shint liczba, void *ptrs[])
 {shint no, wp, i, y_shift=0, xkursor, ykursor, yd, mod;
	shint X_akt0, XX_max, Y_akt0, YY_max;
	struct okno *ok, *Ok0;
	unsigned int attr, attr_d, attr_old;
	if(nr_blank<0) return;
	xkursor=m_wherex(); ykursor=m_wherey();	attr_old=attryb; setcursor(nocursor);
	X_akt0=x_akt0; XX_max=x_maxOkna; Y_akt0=y_akt0; YY_max=y_maxOkna;
...
DISP: wp=ok->Wpis; mod=ok->mod;
			...
			continue;
DD:		attr=attr_o[mod];  attr_d=attr_dat[mod];
			if(ok->ochrona>Uprawn) {attr=attr_inf; wp=2;}
			if(ok->blok_wpisu==1) attr=attr_o[0];
/*			xd=x_akt; yd=y_akt; */
...
/*	term_cur(yd,xd); */
	attryb=attr_old; setcursor(cursor);
	x_akt0=X_akt0; x_maxOkna=XX_max;	y_akt0=Y_akt0; y_maxOkna=YY_max;
...
shint otworz_blankiet(signed char kolor, signed char ramka, shint yp,
																					shint xp, shint ym, shint xm, signed char kod_raportu,
                                          shint nr_rekordu, char czynny)
 {shint i, l, ret, nr_rap, l_danych;
	unsigned int attr, attr_bl, tlo, att, att_d;
	struct okno *ok;
	struct reports *R;
  char *kod_upr[6]={"0 tylko pokaz danych",
  ...
  R=RAP[i]; nr_rap=i;
	if(xp<X_L0) xp=X_L0; if(xm>MXR_MAX) xm=MXR_MAX;
  if(yp<Y_G0) yp=Y_G0; if(ym>MYR_MAX) ym=MYR_MAX;
  Uprawn=0;
	if(R->uprawnienia>=0)
   {l=4; if(NR_OPER==0) l=5;
		Uprawn=Uprawn_pocz;
    if(R->uprawnienia<Uprawn_pocz) R->uprawnienia=Uprawn_pocz;
		i=R->uprawnienia; if(i>l) i=l; Uprawn=i;
    if(i<l)
		 {znak='0'+i;
      zakres[0]=znak; zakres[2]='0'+l;
...
/* fprintf(mystderr,"\n Przed okno_znak() "); */
  ...
  for(i=0;i<l_danych;i++,ok++)
	 {ok->format=NULL; ok->wpis=0; ok->decyzja=-1;
		ok->Rep=NULL; ok->Rep_help=NULL;
		ok->index_helpu=NULL; ok->size_helpu=0;
		ok->wartosc=NULL;
		for(l=0;l<LHELP;l++)  ok->help[l]=NULL;
	 }
	nr_danej=0;
	if(ramka==1) {xp++; yp++; xm--; ym--;}
	X_maxBlank=xm; Y_maxBlank=ym;
	X_0=xp; Y_0=yp;
...
void Kolory(signed char kolor, unsigned int *attr, unsigned int *tlo,
 ...
 {*attr=TERM_WHITE; *tlo=TERM_BLACK_BG;
...
shint wpis_tabl_float(signed char wpis, shint y, shint x0,
                  unsigned int attr_d, unsigned int attr,
                  unsigned int attrtx, shint *index, char *tabl, shint dim,
                  shint L_kol, shint size, char precyzja, char *monit_pocz, shint ind_min,
                  shint ind_max, float w_min, float w_max, char *format_tabl,
...
shint okno_tabl_float(signed char wpis, shint y, shint x0, char *monit_pocz,
               ...
               char *format_tabl, shint *index, float tabl[],
               shint dim, shint L_kol, shint size, char prec)
...
shint okno_double(signed char wpis, shint y, shint x0, shint size,
 ...
 {double wartn;
  ...
  shint ret, xmax,xp, ins, i, k, im=0, x_konc, zn, xa, txl, iobl, dltn=0;
	unsigned int attr_t;
  if(bez_potwierdzania==0) dltn=strlen(taknie);
   wartn=*wart;
   z=szukaj(monit_pocz,'%');
   if(z!=NULL)
    {c=szukaj(z+1,'%');
     if(c!=NULL) {c=szukaj(c,'e');  if(c==NULL) z=NULL; else c++;}
     else z=NULL;
    }
   if(z!=NULL)
    {sprintf(monit,monit_pocz, w_min,w_max);
		 for(cc=z,zz=&zakres[3];cc!=c;cc++,zz++) *zz=*cc;
     zakres[0]='%'; zakres[1]='s'; zakres[2]='(';
     *zz=')'; zz++; *zz='%'; zz++; *zz='s'; zz++; *zz=EOS;
    }
   else {sprintf(monit,monit_pocz); zakres[0]=EOS;}
   if(monit[0]=='#' || monit[0]=='*' || monit[0]=='+') im=1;
   if(monit[1]=='#' || monit[1]=='*' || monit[1]=='+') im=2;
   ms=strlen(monit);  text[0]=EOS;
   for(i=ms-2;i>=im;i--)
   ...
   if(format==NULL) return -1; lwmall++;
   form=&format[13]; format[12]=EOS;
   bufor=&form[ms+3*sizeof(char)]; bufor[0]=EOS;
   sprintf(format," %d.%de",size,precyzja); format[0]='%';
   iobl=1;
   if(monit[0]!='#' && monit[1]!='#') iobl=0;
   if(iobl==0 || wpis==2) sprintf(bufor,format,*wart);
   Okno(y,x0,y,xmax,attr); x_konc=xmax+txl;
   if(wpis!=1)
    {term_type(y,x0,&monit[im],0,attrtx);
     ...
     if(iobl==0 && (wartn<w_min || wartn>w_max))
			{attr_t=ATTR_A; wpis=0; if(txl>1) text[1]='!';}
     if(wpis==0)
      {for(i=0;i<txl;i++)
    ...
    {for(i=0;i<txl;i++)
			{if(i+xmax+1<X_maxBlank) term_type(y,xmax+1+i,&text[i],1,attrtx);}
     x_konc=m_wherex();
     sprintf(form,"%s e",monit); form[ms]='%';
WPIS:ins=INSERT; INSERT=1;
     Okno(y,x0,y,xmax,attr);
     ret=bscanf(bufor,attrtx,form,&wartn);
     INSERT=ins;
     if(ret<0) {Free(format); lwmall--; return -x_konc;}
     if(wartn<w_min || wartn>w_max)
      {sprintf(text,format,wartn); i=strlen(text)+1;
       if(zakres[0]==EOS) sprintf(&text[i]," ZAKRES !! <ENT>-popraw ");
       else sprintf(&text[i],zakres," poza ",w_min,w_max," <ENT> ");
			 i=strlen(text)+1; if(x0+i>X_maxBlank) {i=X_maxBlank-x0; text[i]=EOS;}
       xa=x_konc-i; if(xa<x0) xa=x0;
       k=term_save_image(y,xa,text,0);
       buffer=(char *)Malloc(i*k+1);
       if(buffer!=NULL)
        {term_save_image(y,xa,buffer,i); lwmall++;
				 term_type(y,xa," ",1,ATTR_A); term_type(y,xa+1,text,0,ATTR_A);
        }
       GET_char();
       if(buffer!=NULL)
        {term_restore_image(y,xa,buffer,i); Free(buffer); lwmall--;}
       wartn=*wart; goto WPIS;
      }
     if(ret==0 || wartn==*wart)
      {*wart=wartn; Free(format); lwmall--; return 0;}
     ...
     do{buffer=(char *)Malloc(dltn*k+1);
        if(buffer!=NULL)
         {term_save_image(y,xmax+1,buffer,dltn); lwmall++;}
        term_type(y,xmax+1,taknie,0,attrtx);
				i=m_wherex(); if(i<MXR_MAX) i--; i--; term_cur(y, i); term_flush();
        zn=GET_char(); zn=zn&0x00ff;
        if(buffer!=NULL)
        ...
        if(zn=='n') goto WPIS;
        if(zn=='t')
TAK:     {*wart=wartn; term_printf(y,xmax-size+1, attr,format,wartn);}
       }while(zn!=Esc && zn!='t');
    }
   Free(format); lwmall--;
   return x_konc;
...
shint okno_grupy_posr(signed char wpis, shint y, shint x0, shint size,
               unsigned int attr_d, unsigned int attr,
               unsigned int attrtx, shint *wart, char *index_helpu,
               shint size_helpu, char *monit_pocz,
               shint w_min, shint w_max, shint skok, void *R, void *Rh, char **help)
 {int ret;
  static shint index=0;
...
shint okno_int_menu(signed char wpis, shint y, shint x0, shint size,
               unsigned int attr_d, unsigned int attr,
               unsigned int attrtx, shint *wart, char *monit_pocz,
               shint w_min, shint w_max, shint l_poz_menu, char **menu)
 {int ret;
  static shint index=0;
   ret=wpis_tabl_int(wpis, y, x0, monit_pocz, 0,0, w_min, w_max,
...
shint wpis_tabl_int(signed char wpis, shint y, shint x0, char *monit_pocz,
               ...
               char *format_tabl, shint *index, char *tabl,
               char *index_helpu, shint size_helpu, shint dim,
               shint size, shint skok, void *R, void *Rh, char **help)
 {shint wartn, *wart, ind_size, dl_zakr, W_min, W_min_wpis, index_h, W_max;
  ...
  shint xmax, xp, xp_ind, ret, nr_zm, xp_naz, ins, i, k, x_konc, txl;
  shint iobl, dltn=0, dx_ind=0, x_k_help, zn, lsmax;
  unsigned int attr_t;
  char blok=0;
   if(wpis<-2) {blok=1; wpis=-wpis;}
   W_min=w_min; W_min_wpis=w_min; grupa=0;
/* tak bylo do 21.08.98        if(typ=='g' || typ=='G') grupa=1; */
/* ujemna tylko dla G          if(typ=='G') {W_min=-9999; typ='g';} */
   if(typ=='g' || typ=='G')
    {grupa=1; W_min=-9999; typ='g';
     if(w_min<0) {W_min_wpis=w_min+1; w_min=0;}
   ...
   if(fmon!=NULL)
    {c=szukaj_formatow(fmon,'d',&f_pocz);
     if(c==NULL) sprintf(&opis_danej[dl_zakr],fmon);
     else sprintf(&opis_danej[dl_zakr],fmon, w_min,w_max);
    }
   lsmax=0;
   xmax=wydruk_monitu(wpis, opis_danej, dl_zakr, taknie, &dltn, text, &x0,
   ...
   lwmall++;
   if(size>0) {sprintf(format," %dd",size); format[0]='%';}
   else format[0]=0;
   form=&format[6];
   bufor=&form[6*sizeof(char)]; bufor[0]=EOS;
   iobl=1;
   if(hash!='#') iobl=0;
   wart=(shint*)(tabl+(*index)*dim);
   wartn=*wart;
   if(grupa>0) {if(wartn<W_min) wartn=W_min; if(wartn>w_max) wartn=w_max;}
/*   if(help!=0 && skok>0)
    {if(wpis==0 && wartn>skok) {wartn=skok-1; *wart=wartn;}}
*/
   if(iobl==0 || wpis==2 && size>0) sprintf(bufor,format,wartn);
   xp_ind=xp;
   if(ind_size>0)
    {xp_ind=wydruk_indexu(y,xp_naz,attr,attrtx,f_nazwy,f_ind,*index,fk_nazwy);
     Okno(y,xp_ind,y,xmax,attr);
   ...
   if(abs(wpis)!=1 && wpis!=-2)
    {if(size>0) {term_type(y,xp,bufor,0,attr_d); x_k_help=m_wherex();}
     else x_k_help=xp;  
     x_konc=x_k_help;
     if(help!=NULL)
      {if(grupa>0)
				    {index_h=wartn; W_max=w_max; err=0;
    				 if(size_helpu>0)
  					   {long int skok_sum;
     					 skok_sum=ustal_skok_adr(Rh, wartn, size_helpu, &err);
			     		 if(err==0)
						     index_h=*(shint*)((unsigned long int)index_helpu+skok_sum);
   					   W_max=9999;
			   		  }
  				   if(err>0)
    ...
    {x_konc=xmax+1; HELP=0;
     if(help!=NULL)
      {if(grupa>0)
        ...
        {menu=zle; if(wartn<skok && wartn>=0) menu=help[wartn];
         term_type(y,xmax+1," ",1,attrtx);
/*         for(i=0;i<skok;i++)
					{ls=strlen(help[i]); if(ls>lsmax) lsmax=ls;}
*/
...
/* -------------------- Obsluga wpisu indeksu ---------------------- */
     if(ind_size>0 && wpis==-1)
       ...
       if(ret<0)
        {*index=i; Free(format); HELP=0; lwmall--;
/*         term_printf(y,xp_ind,attr,i); term_printf(y,xp,attr,*wart); */
       ...
       if(*index!=i)
        {wart=(shint *)(tabl+(*index)*dim);
         wartn=*wart;
         if(iobl==0) sprintf(bufor,format,wartn);
				}
       wpis=1;
       if(help!=NULL) HELP=1; else HELP=0;
      }
/* ----------------------------------------------------------------- */
WPIS:do
      {ins=INSERT; INSERT=1;
...
/*         if(wartn<W_min) *wart=W_min;
       ...
       if(wartn<W_min_wpis || wartn>w_max)
        {wartn=nr_zm; sprintf(bufor,format,wartn);
         if(HELP==2) HELP=1; goto WPIS;
        }
       if(help!=NULL)
					   {if(grupa>0)
          {index_h=wartn; W_max=w_max; err=0;
...
*/
     if(wartn==*wart)
      {*wart=wartn; Free(format); HELP=0; lwmall--; return 0; /*x_konc;*/}
     term_printf(y,xmax-size+1, attr,format,wartn);
     if(bez_potwierdzania==1) {zn='t'; goto TAK;}
     k=term_save_image(y,x_konc,taknie,0);
     do{buffer=(char *)Malloc(dltn*k+1);
        if(buffer!=NULL)
         {term_save_image(y,x_konc,buffer,dltn); lwmall++;}
        term_type(y,x_konc,taknie,0,attrtx);
				i=m_wherex(); if(i<MXR_MAX) i--; i--; term_cur(y, i); term_flush();
        zn=GET_char(); zn=zn&0x00ff;
				if(buffer!=NULL)
   ...
   xmax=xo+dl;
	 if(xmax>X_maxBlank)
		{if(yo<=Y_maxBlank) return 0;
		 xo+=(X_maxBlank-xmax); xmax=X_maxBlank;
     if(xo<X_L0) {xo=X_L0; dl=xmax-xo;}
    }
...
/************************************************************************/
shint disp_dana(signed char wp, shint y0, shint x,
								unsigned int attr_D, unsigned int attr_o,
								unsigned int attrtx, struct okno *ok, shint y_shift, char mod)
 {shint ret, y, *ind;
...
*/
	 ...
	 switch(ok->typ)
			 ...
			 case 'w':
				 ...
				 break;
			 case 'F':
				 if(ok->help[2]!=NULL) {ind=(shint *)ok->help[2];} /* attr_d=attr_D;} */
				 else ind=&(ok->index);
...
/*shint wpis_tabl_int(signed char wpis, shint y, shint x0, char *monit_pocz,
										...
										char *format_tabl, shint *index, char *tabl, shint dim,
										shint size, shint skok, char **help); */
				 ret=wpis_tabl_int(wp, y, x, ok->format, 0, 0,
							 *(shint*)(ok->wmin_opcje), *(shint*)(ok->wmax), attr_d, attr_o,
...
/* =================================================================== */
shint dana_float(shint y, shint x, char *monit, float *w_min, float *w_max,
...
shint dana_float_wzgl(shint y, shint x, char *monit, float *w_min,
                 float *w_max, float *wart, shint size, char precyzja,
...
shint dana_float_wzgl_dec(shint y, shint x, char *monit, float *w_min,
                 float *w_max, float *wart, shint size, char precyzja,
                 shint ochrona, char kod_animacji, float *baza, shint kod_decyzji)
 {shint ret;
...
shint tabl_float_wzgl(shint y, shint x, char *monit, shint ind_min, shint ind_max,
                    float *w_min, float *w_max, char *format_tabl,
...
shint def_float(shint y, shint x, char *monit, shint ind_min, shint ind_max,
              float *w_min, float *w_max, char *format_tabl, shint *index,
              float *tablica, shint dim, shint L_kol, shint size, char precyzja,
              shint ochrona, char kod_animacji, float *baza, char *format,
              float *abs_min, float *abs_max)
 {struct okno *ok;
	 if(Ok==NULL)
		{komunikat(MY_MAX,X_L0,ATTR_A," Brak adresu dla danej %d blankietu. <Ent> ",
...
shint wpis_tabl_float(signed char wpis, shint y, shint x0,
               unsigned int attr_d, unsigned int attr, unsigned int attrtx,
               shint *index, char *tabl, shint dim, shint L_kol, shint size,
               char precyzja, char *monit_pocz, shint ind_min, shint ind_max,
 ...
 {float *wart, wartn[30], wmin, wmax, w_abs;
  ...
  shint ret, xmax, xp, xp_ind, xp_naz, i, k, x_konc;
  shint dlh_ogr, ll, l, j, n, poz_wart_abs=0, txl, iobl=0, dltn=0;
  shint dx_ind, ind_size, dl_zakr, xpp, nrkol, xmax_tab;
  unsigned int attr_t;
   typ='f';
   if(format_ogr==NULL && fabs(baza)>1.e-4)
		{w_min=baza*w_min; w_max=baza*w_max;}
...
/* ------------ koniec przeniesienia ------------------------------- */
   ...
   dlh_ogr=0; help_ogr[0]=EOS;  znak=EOS;
   if(format_ogr!=NULL)
    {format=format_ogr; zakr_abs=&help_ogr[MXR_MAX]; *zakr_abs=EOS;
		 i=0; l=0; j=0;
     for(k=0;k<2;k++)
      {for(fh=format;*fh!='%';fh++,j++) {help_ogr[l]=format_ogr[j]; l++;}
       if((*fh)==EOS) {format=NULL; break;}
       help_ogr[l]=EOS; l++;
       typ=czy_format(fh,&i);
       if(typ==EOS) continue;
       if(typ!='f')
       ...
       fh[i]=znak;
       format=&fh[i];
       for(n=0;n<i;n++) {help_ogr[l]=format_ogr[j]; l++; j++;}
       help_ogr[l]=EOS; l++;
   ...
   {shint siztab;
    ...
    size=(siztab+1)/L_kol-1; xmax=xmax_tab-(size+1)*(L_kol-1);
   }
	 if(xmax_tab==0) return 0;
   format=(char *)Malloc((size+40)*sizeof(char));
   if(format==NULL) return -1;
   lwmall++;
   sprintf(format," %d.%df",size,precyzja); format[0]='%';
   form=&format[11];
   bufor=&form[10*sizeof(char)]; bufor[0]=EOS;
   iobl=1;
   if(hash!='#') iobl=0;
   if(iobl==0 || wpis==2) sprintf(bufor,format,*wart);
   Okno(y,x0,y,xmax_tab,attr); x_konc=xmax_tab+txl;
   x_konc=xmax_tab+1;
   if(dlh_ogr>0)
		{term_type(y,x_konc,help_ogr,0,attrtx);
     term_printf(y,m_wherex(),attrtx,format_bazy,baza);
   ...
   if(abs(wpis)!=1)
    {for(nrkol=0,xpp=xp;nrkol<L_kol;nrkol++,xpp+=(size+1))
      {if(iobl==0 || wpis==2)
       ...
       else {Okno(y,xpp,y,xpp+size,attr_d);clr_Okno();}
/*     x_konc=m_wherex(); */
       attr_t=attrtx;
       if(iobl==0)
...
/* ========================= dla tablicy ========================== */
          ...
          {struct dane_wpisu *dw=NULL;
           if(wpis==4)
            {dw=(struct dane_wpisu *)Malloc(sizeof(struct dane_wpisu));
             sprintf(form,"%c f",hash); form[1]='%';
             dw->x0=x0; dw->xmax=xmax; dw->x_konc=x_konc;
             dw->dltn=dltn; dw->poz_wart_abs=poz_wart_abs;
             dw->dlh_ogr=dlh_ogr;
             dw->format=format; dw->format_bazy=format_bazy;
             dw->format_tx=format_tx; dw->form=form;
             dw->format_wyn=format_wyn;
						 dw->help_ogr=help_ogr; dw->format_ogr=format_ogr;
             dw->text=text; dw->zakres=zakres; dw->zakr_abs=zakr_abs;
             dw->bufor=bufor; dw->taknie=taknie; dw->iobl=iobl;
             dw->wartn=wartn; dw->baza=baza; dw->w_min=w_min;
             dw->w_max=w_max; dw->abs_min=abs_min; dw->abs_max=abs_max;
            };
					 ret=okno_help(y+1, xp_naz-1, xmax_tab+1, attrtx, attr, 'f', ind_min,
                 ind_max, (shint)w_min, (shint)w_max, (char*)dw, L_kol, size,
                 precyzja, NULL, NULL, NULL, 0, tabl, dim, f_nazwy, f_ind, fk_nazwy);
	   if(dw!=NULL) Free((char*)dw);
           if(ret>=0)
            {if((ret+ind_min)==(*index)) x_konc=0;
             else *index=ret+ind_min;
						}
     ...
     HELP=0; if(format!=NULL) {Free(format); lwmall--;}
     return x_konc;
    }             /* koniec dla wpis!=1 */
/* ----------------------------------------------------------------- */
   else           /* obsluga dla wpis=1 */
     ...
     if(ind_size>0 && wpis<=-1)
      {i=*index;
       ret=wpis_indexu(y,xp_ind,ind_size,index,(char*)index, L_kol, ind_min,
             ...
             0, NULL, NULL, NULL, 'f', dltn, (shint)w_min, (shint)w_max, dlh_ogr);
       if(ret<0)
    	{*index=i; Free(format); HELP=0; lwmall--;
         return -1;
				}
...
void begin_scroll(shint dana_nr) {SCROLL=dana_nr;}
